...
// Sampul album lokal → lewat proxy watermark (galeri-cover.php), supaya
// gambar yang diindeks/diunduh dari sitemap juga membawa watermark. Foto
// eksternal (URL http penuh) dibiarkan apa adanya.
function xml_cover_url(string $gambar, string $base): string {
    if ($gambar === '') return '';
    if (str_starts_with($gambar, 'http')) return $gambar;
    return $base . '/galeri-cover.php?file=' . rawurlencode($gambar);
}
...
foreach ($topItems as $item) {
    $gambar = $item['Gambar'] ?? '';
    if (!$gambar) continue;
    $imgUrl = xml_cover_url($gambar, $base);
