...
$og_image         = assetUrl('og-preview.webp');
...
// Deteksi status login admin (untuk avatar + dropdown di menu, ganti tombol Login)
require_once __DIR__ . '/components/header_auth.php';
...
<!-- Preload LCP images -->
<link rel="preload" as="image" href="https://img.parokitulungagung.org/assets/parokitulungagung.webp" fetchpriority="high">
...
/* Admin account widget (avatar + dropdown), menggantikan tombol Login saat sudah login */
#login-portal-wrap { position: relative; }
.admin-account { position: relative; }
.admin-account-trigger { display: inline-flex; align-items: center; gap: 6px; background: rgba(255,255,255,.06); border: 1.5px solid rgba(201,162,58,.4); border-radius: 30px; padding: 3px 8px 3px 3px; cursor: pointer; transition: all .25s ease; }
.admin-account-trigger:hover { background: rgba(201,162,58,.18); border-color: rgba(201,162,58,.85); }
.admin-account.open .admin-account-trigger { background: rgba(201,162,58,.22); border-color: rgba(201,162,58,.9); }
.admin-avatar { position: relative; width: 30px; height: 30px; flex-shrink: 0; }
.admin-avatar-face { width: 100%; height: 100%; border-radius: 50%; overflow: hidden; background: linear-gradient(135deg,#c9931a,#f0c040); display: flex; align-items: center; justify-content: center; box-shadow: 0 0 0 2px rgba(255,255,255,.15); }
.admin-avatar img { width: 100%; height: 100%; object-fit: cover; display: block; }
.admin-avatar-initial { font-family: var(--font-sans); font-weight: 800; font-size: 13px; color: #1c0e00; }
.admin-status-dot { position: absolute; bottom: -1px; right: -1px; width: 9px; height: 9px; border-radius: 50%; background: #3bc46a; border: 2px solid #1c0c04; z-index: 1; }
.admin-chevron { stroke: #f5d98a; transition: transform .25s ease; flex-shrink: 0; }
.admin-account.open .admin-chevron { transform: rotate(180deg); }
.admin-dropdown { position: absolute; top: calc(100% + 10px); right: 0; min-width: 230px; background: linear-gradient(160deg,#1c0c04 0%,#2e1608 100%); border: 1px solid rgba(201,162,58,.35); border-radius: 14px; box-shadow: 0 16px 40px rgba(0,0,0,.5); padding: 8px; opacity: 0; visibility: hidden; transform: translateY(-8px) scale(.97); transform-origin: top right; transition: opacity .2s ease, transform .2s ease, visibility .2s; z-index: 1100; }
.admin-account.open .admin-dropdown { opacity: 1; visibility: visible; transform: translateY(0) scale(1); }
.admin-dropdown-header { display: flex; align-items: center; gap: 10px; padding: 8px 8px 12px; }
.admin-dropdown-header .admin-avatar-lg { width: 42px; height: 42px; }
.admin-dropdown-header .admin-avatar-lg .admin-avatar-initial { font-size: 17px; }
.admin-dropdown-id { min-width: 0; }
.admin-dropdown-name { font-family: var(--font-sans); font-weight: 700; font-size: 13.5px; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 150px; }
.admin-dropdown-role { font-family: var(--font-sans); font-size: 11px; color: #d4af6a; letter-spacing: .03em; margin-top: 1px; }
.admin-dropdown-sep { height: 1px; background: rgba(201,162,58,.25); margin: 2px 4px 6px; }
.admin-dropdown-item { display: flex; align-items: center; gap: 10px; padding: 9px 10px; border-radius: 9px; color: #f0e6d2; font-family: var(--font-sans); font-size: 13px; font-weight: 600; text-decoration: none; transition: all .18s ease; }
.admin-dropdown-item svg { stroke: #d4af6a; flex-shrink: 0; transition: stroke .18s ease; }
.admin-dropdown-item:hover { background: rgba(201,162,58,.2); color: #fff; }
.admin-dropdown-item:hover svg { stroke: #fff; }
.admin-dropdown-item-danger { color: #ff9d8a; }
.admin-dropdown-item-danger svg { stroke: #ff9d8a; }
.admin-dropdown-item-danger:hover { background: rgba(230,58,0,.18); color: #ffb3a2; }
.admin-dropdown-item-danger:hover svg { stroke: #ffb3a2; }
@media (max-width: 420px) {
  .admin-dropdown { position: fixed; top: auto; bottom: 0; left: 0; right: 0; border-radius: 18px 18px 0 0; min-width: 0; transform: translateY(100%); }
  .admin-account.open .admin-dropdown { transform: translateY(0); }
}
...
/* Cinematic light rays sweeping slowly across the hero */
.site-hero__rays {
  ...
  background: conic-gradient(from 0deg at 50% 28%,
    transparent 0deg, rgba(240,217,138,.10) 6deg, transparent 18deg,
    transparent 158deg, rgba(240,217,138,.07) 170deg, transparent 188deg,
    transparent 336deg, rgba(240,217,138,.09) 350deg, transparent 360deg);
  animation: raySweep 60s linear infinite;
}
@keyframes raySweep { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }

/* Floating gold embers / incense dust rising through the hero */
.site-hero__particles { position: absolute; inset: 0; z-index: 2; overflow: hidden; pointer-events: none; }
.particle {
  position: absolute; bottom: -20px; border-radius: 50%; opacity: 0;
  background: radial-gradient(circle, rgba(250,226,152,.95) 0%, rgba(250,226,152,0) 72%);
  box-shadow: 0 0 6px rgba(250,226,152,.6);
...
@keyframes emberFloat {
  ...
  8%   { opacity: .9; }
  85%  { opacity: .35; }
  100% { opacity: 0; transform: translate(var(--drift,20px), -560px) scale(1); }
}

/* Thin gold corner frame, fades in like an illuminated manuscript border */
.site-hero__frame { position: absolute; inset: 20px; z-index: 4; opacity: 0; pointer-events: none; animation: frameFadeIn 1.6s ease .5s both; }
@keyframes frameFadeIn { from { opacity: 0; } to { opacity: .55; } }
.site-hero__corner { position: absolute; width: 32px; height: 32px; border-color: rgba(201,162,58,.6); }
.site-hero__corner--tl { top: 0; left: 0; border-top: 1px solid; border-left: 1px solid; }
.site-hero__corner--tr { top: 0; right: 0; border-top: 1px solid; border-right: 1px solid; }
.site-hero__corner--bl { bottom: 0; left: 0; border-bottom: 1px solid; border-left: 1px solid; }
.site-hero__corner--br { bottom: 0; right: 0; border-bottom: 1px solid; border-right: 1px solid; }

/* Soft halo glow breathing behind the title, like candlelight */
.site-hero__halo {
  position: absolute; top: 48%; left: 50%; width: 360px; height: 360px; z-index: -1;
  transform: translate(-50%,-55%); border-radius: 50%; filter: blur(4px); pointer-events: none;
  background: radial-gradient(circle, rgba(240,217,138,.18) 0%, rgba(240,217,138,.06) 45%, transparent 72%);
...
/* Each hero element cascades up individually via its own --d delay */
.site-hero__anim { opacity: 0; transform: translateY(24px); animation: heroFadeUp .9s cubic-bezier(.22,1,.36,1) both; animation-delay: var(--d, 0s); }
...
.site-hero__title {
  ...
  animation: heroFadeUp .9s cubic-bezier(.22,1,.36,1) var(--d,0s) both,
             heroTitleGlow 6s ease-in-out var(--d,0s) infinite alternate;
}
...
.site-hero__scroll {
  position: absolute; left: 48.5%; bottom: 24px;
  transform: translateX(-50%); z-index: 3;
  display: flex; flex-direction: column; align-items: center; gap: 8px;
  color: rgba(255,255,255,.7);
  opacity: 0; animation: heroFadeUp .9s cubic-bezier(.22,1,.36,1) 1.15s both;
}
...
.site-hero__mouse-wheel {
  width: 3px; height: 7px; border-radius: 2px;
  background: var(--gold-bright);
  box-shadow: 0 0 6px rgba(240,217,138,.85);
...
.section-heading__ornament svg { flex-shrink: 0; animation: ornamentSpin 8s linear infinite; }
...
/* ================================================================
   ...
   ================================================================ */
.instagram-modern { padding-bottom: 26px; }

/* Bingkai tipis, TANPA kotak putih — menyatu dengan latar halaman */
.ig-home-card {
  width: 100%;
}

/* ── JENDELA CROP INSTAGRAM (AUTO-SCALE) ─────────────────────────
   Widget TagEmbed aslinya berisi (dari atas ke bawah): teks "Powered by",
   tombol "Share your experience", baris 3 tab ikon, BARU slider foto.
   Karena semuanya menumpuk berurutan dan slider ada PALING BAWAH, kita
   "geser ke atas" seluruh isi widget lalu sembunyikan sisanya di luar
   jendela — hasilnya cuma slider yang kelihatan.

   Supaya tampilan di mobile bukan "dipotong beda" tapi benar-benar
   "gambar yang sama, mengecil", seluruh widget dirender di satu lebar
   acuan tetap (--ig-ref-width, sesuai lebar container di desktop),
   lalu di-scale (diperkecil) oleh JavaScript (lihat script
   #igAutoScale di bagian akhir halaman) supaya pas dengan lebar layar
   berapa pun — posisi crop-nya (proporsinya) selalu identik.

   Kalau slider masih terpotong / sisa header masih nongol di DESKTOP,
   cukup ubah 2 angka ini (JS otomatis menyesuaikan skalanya untuk
   layar lain):
     --ig-ref-width = lebar (px) tempat 2 angka di bawah ini dikalibrasi
     --ig-frame-h   = tinggi jendela yang terlihat, pada lebar acuan itu
     --ig-shift     = seberapa jauh didorong ke atas, pada lebar acuan itu
*/
.ig-home-frame {
  --ig-ref-width: 880px;
  --ig-frame-h: 300px;
...
.ig-home-card-body {
  ...
  transform: translateY(calc(-1 * var(--ig-shift))); /* fallback sebelum JS jalan */
}
.ig-home-card-body,
...
/* ================================================================
   SCROLL REVEAL (fade + rise into view as the page is scrolled)
   ================================================================ */
.reveal-up { opacity: 0; transform: translateY(30px); transition: opacity .75s cubic-bezier(.22,1,.36,1), transform .75s cubic-bezier(.22,1,.36,1); }
.reveal-up.is-visible { opacity: 1; transform: translateY(0); }
.quick-nav-card.reveal-up:nth-child(1) { transition-delay: .05s; }
.quick-nav-card.reveal-up:nth-child(2) { transition-delay: .15s; }
.quick-nav-card.reveal-up:nth-child(3) { transition-delay: .25s; }
...
.romo-card.modern { position: relative; flex: 0 0 200px; width: 200px; padding: 0 0 24px; border-radius: var(--r-lg); background: #fff; border: 1px solid rgba(201,162,58,.2); box-shadow: var(--shadow-sm); transition: transform .35s var(--ease-spring),box-shadow .35s ease; overflow: visible; text-decoration: none; color: inherit; display: block; cursor: pointer; }
...
@media (max-width: 768px) {
  .site-hero__bg { position: absolute; top: 0; height: 100%; }
  .site-hero { height: calc(100vh - var(--navbar-h)); height: calc(100dvh - var(--navbar-h)); min-height: 460px; }
...
@media (max-width: 680px) {
  .site-hero { height: calc(100vh - var(--navbar-h)); height: calc(100dvh - var(--navbar-h)); min-height: 400px; }
  ...
  /* --- PERBAIKAN INSTAGRAM MOBILE (FULL-BLEED & ZOOM-IN) --- */
  .content-section.instagram-modern {
    margin-left: -16px;
    margin-right: -16px;
    padding-left: 0;
    padding-right: 0;
  ...
  .content-section.instagram-modern .section-heading {
    padding-left: 16px;
    padding-right: 16px;
  }
  .ig-home-frame {
    border-radius: 0;
    /* Memanipulasi kalkulasi JS: Angka acuan diperkecil agar skala zoom membesar */
    --ig-ref-width: 420px; 
  }
  .ig-home-card-body {
    /* Kunci dimensi asli agar layout internal TagEmbed & posisi crop tetap sempurna */
    width: 880px !important;
    /* Pusatkan titik zoom di tengah agar kartu utama tidak tergeser ke kanan/kiri */
    left: 50% !important;
    margin-left: -440px !important;
    transform-origin: top center !important;
  }
  
  /* --- PERBAIKAN TOMBOL IKUTI DI INSTAGRAM --- */
  .ig-home-follow-wrap {
    margin-top: -45px; /* Tarik tombol ke atas untuk menutupi celah kosong akibat frame scaling */
    position: relative;
    z-index: 5; /* Pastikan tombol tetap bisa diklik jika tumpang tindih dengan shadow transparan */
  }
  .ig-home-follow {
    padding: 10px 22px; /* Dibuat lebih compact agar proporsional di mobile */
    font-size: .55rem;
  }
  /* --------------------------------------------------------- */
...
<?php include __DIR__ . '/components/loading_screen.php'; ?>
    ...
    <div>
      <div id="divmenu" class="menutombol" style="display:none">
        <div class="menu-wrapper">
          <a href="/" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_home1.png') ?>" alt="Beranda" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Home</span></a>
          <a href="/agenda" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_petugas.png') ?>" alt="Info Paroki dan Agenda" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Info/Agenda</span></a>
          <a href="/jadwal-misa" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_jadwal_misa.png') ?>" alt="Jadwal Misa" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Jadwal Misa</span></a>
          <a href="/galeri" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_foto.png') ?>" alt="Galeri Foto" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Galeri</span></a>
          <a href="/stories" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_kronik.png') ?>" alt="Stories" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Stories</span></a>
          <a href="/profil-lingkungan" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_lingkungan.png') ?>" alt="Wilayah Lingkungan" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Wilayah</span></a>
          <a href="/profil-ai" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_ai.png') ?>" alt="Asisten Imam" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Asisten Imam</span></a>
          <a href="/profil-dpp" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_dpp.png') ?>" alt="DPP & BGKP" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">DPP &amp; BGKP</span></a>
          <a href="/kategorial" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_kategorial3.png') ?>" alt="Kategorial" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Kategorial</span></a>
          <a href="/e-lonceng" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_e-lonceng.png') ?>" alt="E-Lonceng" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">E-Lonceng</span></a>
          <a href="/artikel/berita" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_berita.png') ?>" alt="Artikel, Kronik, dan Historia" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Artikel</span></a>
          <a href="/umkmumat" class="divtombol"><img class="imgtombol" src="<?= iconUrl('umkm.png') ?>" alt="Pasar Umat" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">Pasar Umat</span></a>
          <a href="/komedi" class="divtombol"><img class="imgtombol" src="<?= iconUrl('icon_square_komedi.png') ?>" alt="KoMeDi" width="48" height="48" loading="lazy" decoding="async"><span class="tulisan">KoMeDi</span></a>
        </div>
      </div>
    </div>
  </div>
...
<header class="site-hero" id="siteHero" aria-label="Paroki SMDTBA Tulungagung">
  ...
  <div class="site-hero__rays" aria-hidden="true"></div>
  <div class="site-hero__overlay" aria-hidden="true"></div>
  <div class="site-hero__noise" aria-hidden="true"></div>
  <div class="site-hero__particles" id="heroParticles" aria-hidden="true"></div>

  <div class="site-hero__frame" aria-hidden="true">
    <span class="site-hero__corner site-hero__corner--tl"></span>
    <span class="site-hero__corner site-hero__corner--tr"></span>
    <span class="site-hero__corner site-hero__corner--bl"></span>
    <span class="site-hero__corner site-hero__corner--br"></span>
  </div>

  <div class="site-hero__content">
    <div class="site-hero__halo" aria-hidden="true"></div>
    <p class="site-hero__eyebrow site-hero__anim" style="--d:.15s">Gereja Katolik &middot; Keuskupan Surabaya</p>
    <h1 class="site-hero__title site-hero__anim" style="--d:.35s" aria-label="Paroki Tulungagung">PAROKI TULUNGAGUNG</h1>
    <p class="site-hero__subtitle site-hero__anim" style="--d:.55s">Santa Maria Dengan Tidak Bernoda Asal</p>
    <div class="site-hero__divider site-hero__anim" style="--d:.7s" aria-hidden="true">
      <span></span>
    ...
    <div class="site-hero__actions site-hero__anim" style="--d:.9s">
  ...
  <div class="site-hero__scroll" aria-hidden="true">
    ...
    <span class="site-hero__mouse"><span class="site-hero__mouse-wheel"></span></span>
  </div>
</header>
...
<div class="container-main">
  ...
  <!-- ARTIKEL SECTION -->
  <div class="content-section reveal-up">
    <div class="section-heading">
      <div class="section-heading__ornament" aria-hidden="true">
        <span></span>
        <svg width="9" height="9" viewBox="0 0 9 9" fill="currentColor">
      ...
      <h2>Warta Paroki Tulungagung</h2>
  ...
  <section class="quick-nav-section" aria-label="Menu Cepat">
    ...
    <nav class="quick-nav">
      <a href="/agenda" class="quick-nav-card reveal-up" aria-label="Lihat Agenda Paroki">
      ...
      <a href="/artikel/berita" class="quick-nav-card reveal-up" aria-label="Baca Artikel Paroki">
        <div class="quick-nav-icon" aria-hidden="true">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
      ...
      <a href="/galeri" class="quick-nav-card reveal-up" aria-label="Lihat Galeri Paroki">
        <div class="quick-nav-icon" aria-hidden="true">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
  ...
  <div class="content-section romo-modern" id="secRomo" style="display:none; padding-bottom: 32px;">
    ...
    <div class="romo-wrapper" id="romoWrapper"></div>
  </div>
...
<div class="ft-modal-backdrop" id="footerModalTeam" onclick="ftModalClose('footerModalTeam', event)">
  <div class="ft-modal-card">
    <div class="ft-modal-header">
      ...
      <img class="ft-modal-logo" src="https://img.parokitulungagung.org/assets/smdtba-sharelogo.webp" alt="SMDTBA Logo" width="192" height="192" loading="lazy" decoding="async">
...
<div class="ft-modal-body">
  ...
  <p class="ft-modal-dev" id="devCredit"></p>
</div>
...
(() => {
    ...
    const creditText = d("RGlrZW1iYW5na2FuIG9sZWg="); // Dikembangkan oleh
    const developer  = d("S2F2ZW50YXJh");             // Kaventara
    const link       = d("L3JlZGlyZWN0LnBocA==");     // /redirect.php

    const container = document.getElementById("devCredit");
  ...
  <div class="ft-grid">
    <div class="ft-col">
      ...
      <ul class="ft-nav">
        ...
        <li><a href="/penulis">Kontributor</a></li>
    ...
    <div class="ft-col">
      ...
      <div class="ft-info-item">
        ...
        <span class="ft-info-text">Jl. Ahmad Yani Tim. Gg. IV No.1, Bago, Kec. Tulungagung, Kabupaten Tulungagung, Jawa Timur 66218</span>
      </div>
      <div class="ft-info-item">
...
/* ── HERO FLOATING EMBERS / GOLD DUST ─────────────────────────── */
(function(){
  var wrap=document.getElementById('heroParticles');
  if(!wrap)return;
  var reduceMotion=window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if(reduceMotion)return;
  var count=window.innerWidth<=600?9:20;
  var frag=document.createDocumentFragment();
  for(var i=0;i<count;i++){
    var p=document.createElement('span');
    p.className='particle';
    var size=(Math.random()*3+1.4).toFixed(1);
    var left=(Math.random()*100).toFixed(1);
    var duration=(Math.random()*10+11).toFixed(1);
...
/* ── INSTAGRAM AUTO-SCALE (crop proporsional di semua lebar layar) ─
   Widget TagEmbed dirender pada lebar acuan tetap (--ig-ref-width),
   lalu di-scale-down sesuai lebar frame yang sesungguhnya. Dengan
   begitu, gambar yang terlihat "mengecil" persis seperti memperkecil
   screenshot — bukan menampilkan potongan/crop yang berbeda.
*/
(function(){
...
(function(){
  ...
  window.__revealRomo = function(){ reveal('skelRomo','secRomo'); };
  window.__hideRomoSkel = function(){
    var skel=document.getElementById('skelRomo');
    if(skel) skel.classList.add('hp-hidden');
  };
  runIdle(function(){ loadRomoDinamis(); });
})();

/* ── ROMO PAROKI DINAMIS (Supabase) ───────────────────────────── */
function loadRomoDinamis(){
  var SB_URL='<?= htmlspecialchars($_sb_url,  ENT_QUOTES) ?>';
  var SB_KEY='<?= htmlspecialchars($_sb_anon, ENT_QUOTES) ?>';
  var FALLBACK_IMG='https://img.parokitulungagung.org/assets/parokitulungagung.webp';
  // URL CDN untuk foto person (R2). Default fallback: /person/<file> (relatif)
  var PERSON_CDN_URL = <?= json_encode(rtrim(R2_CDN_URL, '/')) ?> + '/person';
  window.PERSON_CDN_URL = PERSON_CDN_URL;

  function esc(s){return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');}
  function today(){return new Date().toISOString().slice(0,10);}

  var url=SB_URL+'/rest/v1/romo_paroki'
    +'?select=nama,slug,jabatan,foto,tanggal_mulai,tanggal_selesai,urutan'
    +'&aktif=eq.true'
    +'&order=urutan.asc,tanggal_mulai.desc';
  ...
  .then(function(data){
    var t=today();
    var active=(Array.isArray(data)?data:[]).filter(function(r){
      if(r.tanggal_mulai && r.tanggal_mulai>t) return false;
      if(r.tanggal_selesai && r.tanggal_selesai<t) return false;
      return true;
    });
    var wrap=document.getElementById('romoWrapper');
    if(!wrap || !active.length){ if(window.__hideRomoSkel) window.__hideRomoSkel(); return; }

    wrap.innerHTML = active.map(function(r){
      var src = r.foto ? (window.PERSON_CDN_URL + '/' + esc(r.foto)) : FALLBACK_IMG;
      var nama = esc(r.nama||'');
      return ''
        +'<a class="romo-card modern" href="/romo/'+esc(r.slug)+'">'
        +  '<div class="romo-card-top"></div>'
        +  '<div class="romo-img-wrap"><div class="romo-img-inner">'
        +    '<img src="'+src+'" alt="'+nama+'" width="82" height="82" loading="lazy" decoding="async">'
        +  '</div></div>'
        +  '<div class="romo-card-body">'
        +    '<span class="romo-role">'+esc(r.jabatan||'')+'</span>'
        +    '<p class="romo-name">'+nama+'</p>'
        +    '<div class="romo-ornament"><span></span><i></i><span></span></div>'
        +  '</div>'
        +'</a>';
    }).join('');

    if(window.__revealRomo) window.__revealRomo();
  })
  .catch(function(){ if(window.__hideRomoSkel) window.__hideRomoSkel(); });
}
...
(function(){
  var SB_URL='<?= htmlspecialchars($_sb_url,  ENT_QUOTES) ?>';
  var SB_KEY='<?= htmlspecialchars($_sb_anon, ENT_QUOTES) ?>';
  var FALLBACK_IMG='https://img.parokitulungagung.org/assets/og-preview.webp?v=2';
  ...
  function resolveArtikelThumb(src){
    if(!src || src === 'null' || src === 'undefined' || src.trim() === '') return FALLBACK_IMG;
    var lower = src.toLowerCase();
    if(!lower.match(/\.(jpg|jpeg|png|webp|gif)/)){
      return FALLBACK_IMG;
    }
    if(src.indexOf('http://')===0 || src.indexOf('https://')===0){
      return src.replace(/^https?:\/\/(www\.)?parokitulungagung\.org\/img\/artikel\//i, 'https://img.parokitulungagung.org/artikel/');
    }
    var clean = src.replace(/^\/?(img\/)?artikel\//i, '');
    return 'https://img.parokitulungagung.org/artikel/' + clean.replace(/^\//, '');
  }
  ...
  function renderArtikel(data){
    ...
    top5.forEach(function(a,i){
      ...
      var src=esc(resolveArtikelThumb(a.thumbnail));
      ...
      slidesHtml+='<img src="'+src+'" alt="'+alt+'" width="600" height="450" loading="'+loading+'"'+fp+' decoding="async" onerror="this.src=\'https://img.parokitulungagung.org/assets/og-preview.webp?v=2\';this.onerror=null;">';
    ...
    featured.forEach(function(a){
      ...
      sideHtml+='<img src="'+src+'" alt="'+alt+'" loading="lazy" decoding="async" onerror="this.src=\'https://img.parokitulungagung.org/assets/og-preview.webp?v=2\';this.onerror=null;">';
