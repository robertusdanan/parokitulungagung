...
async function refreshSeatStatus() {
    try {
        ...
        // Jika server return error HTTP, abaikan — jangan crash
        if (!res.ok) {
            console.warn('Gagal refresh seat status: HTTP', res.status);
            return;
        }

        // Pastikan server benar-benar return JSON, bukan halaman error HTML
        // (shared hosting kadang return "<!DOCTYPE html>" saat server overload)
        const contentType = res.headers.get('content-type') || '';
        if (!contentType.includes('application/json')) {
            console.warn('Gagal refresh seat status: respons bukan JSON (content-type:', contentType + ')');
            return;
        }

        const fresh = await res.json();

        // Validasi minimal: fresh harus object, bukan null/array kosong
        if (fresh && typeof fresh === 'object' && !fresh.error) {
            window.seatStatus = fresh;
        }
    } catch (e) {
        // TypeError: Failed to fetch → server sedang overload/down, abaikan saja
        // Error ini tidak perlu sampai ke user
        console.warn('Gagal refresh seat status:', e.message || e);
    }
}
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (isIndexPage) {
        renderTickets();
        setInterval(async () => { await refreshSeatStatus(); renderTickets(); }, 60000);
    }

    if (isSeatSelectionPage) {
        loadBookedSeats();
        // Refresh seatStatus setiap 60 detik agar status closed selalu sinkron
        // (interval 30 detik terlalu agresif untuk shared hosting)
        setInterval(refreshSeatStatus, 60000);
    }
});
...
async function loadBookedSeats() {
    try {
        ...
        if (typeof bookedSeats !== 'undefined') {
            bookedSeats.clear();
            if (typeof blockedSeats !== 'undefined') blockedSeats.clear();

            // Reset zoneSoldCounts
            window.zoneSoldCounts = {};

            tickets.forEach(ticket => {
                // Kursi dengan status='blocked' (diset manual via Supabase) ditandai
                // terpisah agar tampil abu-abu, bukan hitam seperti kursi terjual biasa
                const isBlockedStatus = typeof ticket.status === 'string'
                    && ticket.status.trim().toLowerCase() === 'blocked';

                // 1. Tandai kursi individual (VVIP/VIP) sebagai sold / blocked
                if (ticket.seat_numbers) {
                    try {
                        ...
                        seats.forEach(seatId => {
                            if (isBlockedStatus && typeof blockedSeats !== 'undefined') {
            ...
            // Tandai kursi yang diblokir manual (status='blocked') sebagai abu-abu di UI
            if (typeof blockedSeats !== 'undefined') {
                blockedSeats.forEach(seatId => {
                    const seat = document.querySelector(`[data-seat-id="${seatId}"]`);
                    if (seat) {
                        seat.classList.remove('sold');
                        seat.classList.add('blocked-seat');
        ...
        <div class="premium-ticket-body">
            ...
            <div class="ticket-categories">
                ${window.ticketConfig.map(config => {
                    ...
                    const isSoldOut = !isClosed && (stat.available <= 0);
 return `
    <div class="category-badge" style="border-color:${config.color};${(isClosed || isSoldOut) ? 'opacity:.7;filter:grayscale(.3);' : ''}">
        ...
        ${isSoldOut ? `
        <div class="category-closed-bar" style="background:#fef2f2;border-color:#fecaca;">
            <span class="category-archived-badge" style="background:#ef4444;color:#fff;">&#10060; Habis Terjual</span>
        </div>` : ''}
    </div>`;
                }).join('')}
            </div>
...
async function fetchTicketStats() {
    try {
        ...
        // Kategori yang memilih kursi (VVIP/VIP): hanya dihitung terjual
        // setelah kursi benar-benar dipilih (seat_selection_status === 'completed').
        // Jumlah sold = jumlah seat_numbers yang terisi (bukan jumlah tiket).
        // Kategori tanpa pilih kursi (Kelas 1/Reguler): langsung hitung dari ticket_type.
        const seatPickerTypes = new Set(['vvip', 'vip']);
        ...
        tickets.forEach(ticket => {
            ...
            try {
                ...
                ticketTypes.forEach(type => {
                    ...
                    if (seatPickerTypes.has(type)) {
                        // VVIP/VIP: hitung hanya jika kursi sudah dipilih (completed)
                        if (ticket.seat_selection_status === 'completed' && ticket.seat_numbers) {
                            try {
                                const seats = Array.isArray(ticket.seat_numbers)
                                    ? ticket.seat_numbers
                                    : JSON.parse(ticket.seat_numbers);
                                // Hitung kursi dari tipe ini saja (seat_numbers bisa campuran)
                                // Gunakan total_seats sebagai fallback jika tidak bisa filter per tipe
                                stats[type].sold += 1;
                            } catch (e) { stats[type].sold += 1; }
                        }
                        // Jika masih pending_seat → belum terhitung (kursi belum dipilih)
                    } else {
                        // Kelas 1 / Reguler: langsung hitung saat beli
                        stats[type].sold += 1;
                    }
                });
        ...
        // Total kuota harus dihitung dari open_quota tiap kategori (kuota yang
        // benar-benar dibuka ke publik), BUKAN dari TOTAL_QUOTA statis.
        // Ini menyamakan logika dengan bar "Total Ketersediaan" di renderTickets().
        const totalOpenQuota = window.ticketConfig.reduce((sum, config) => {
            const openQuota = window.seatStatus?.[config.type]?.open_quota ?? config.quota;
            return sum + openQuota;
        }, 0);

        const totalSold      = Object.values(stats).reduce((sum, s) => sum + s.sold, 0);
        const totalAvailable = totalOpenQuota - totalSold;
...
(function () {
    ...
    form.addEventListener('submit', async function (e) {
        ...
        // Sembunyikan error banner dari percobaan sebelumnya (jika ada)
        hideBookingError();
        ...
        try {
            ...
            // Tampilkan error elegan di dalam modal — form & file tetap utuh, bisa langsung retry
            showBookingError(err.message || 'Terjadi kesalahan saat memproses pemesanan.');
        } finally {
            if (submitBtn)  submitBtn.disabled    = false;
...
// ============================================================
// BOOKING ERROR — tampil di dalam modal, tidak hapus form/file
// ============================================================
function showBookingError(message) {
    // Inject CSS sekali saja
    if (!document.getElementById('bke-style')) {
        ...
        s.textContent = `
            #bookingErrorBanner {
                ...
                border-radius: 14px;
                overflow: hidden;
                animation: bkeSlideIn .3s cubic-bezier(.34,1.4,.64,1);
            }
            @keyframes bkeSlideIn {
            ...
            .bke-glow {
                ...
                background: radial-gradient(ellipse at 50% 0%, rgba(239,68,68,.08) 0%, transparent 65%);
                border-radius: 14px;
            }
            .bke-row1 {
            ...
            .bke-hint {
                font-size: .78rem; color: #64748b; margin-bottom: 12px; line-height: 1.5;
            }
            .bke-hint strong { color: #94a3b8; }
            .bke-actions { display: flex; gap: 8px; }
            .bke-btn-retry {
            ...
            .bke-btn-retry:hover  { opacity: .88; }
            .bke-btn-retry:active { transform: scale(.97); }
            .bke-btn-dismiss {
                padding: 11px 14px;
                background: rgba(255,255,255,.05); border: 1px solid rgba(255,255,255,.1);
                border-radius: 10px; color: #64748b; font-size: .83rem; font-weight: 600;
                cursor: pointer; font-family: inherit; transition: background .15s;
                white-space: nowrap;
            }
            .bke-btn-dismiss:hover { background: rgba(255,255,255,.09); }
        `;
    ...
    if (!banner) {
        ...
        // Sisipkan tepat di atas tombol submit (di dalam .button-group)
        const btnGroup = document.querySelector('#step2 .button-group');
        if (btnGroup) {
            btnGroup.parentNode.insertBefore(banner, btnGroup);
        } else {
            // fallback: sisipkan sebelum submitBtn
            const submitBtn = document.getElementById('submitBtn');
            if (submitBtn) submitBtn.parentNode.insertBefore(banner, submitBtn);
        }
    }

    // Deteksi apakah ini error jaringan / server
    const isNetwork = /fetch|network|failed|timeout/i.test(message);
    const hint = isNetwork
        ? 'Koneksi ke server terputus. <strong>Data Anda aman</strong> — bukti transfer tidak perlu diupload ulang.'
        : 'Cek koneksi internet Anda lalu klik <strong>Coba Lagi</strong> — data & foto sudah tersimpan di form.';

    banner.innerHTML = `
        <div class="bke-inner">
            <div class="bke-glow"></div>
            <div class="bke-row1">
                <div class="bke-icon">
                ...
                <div class="bke-texts">
                    <div class="bke-title">Pemesanan Gagal</div>
                    <div class="bke-msg">${message}</div>
                </div>
            </div>
            <div class="bke-badge">
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                Data & bukti transfer masih tersimpan
            </div>
            <div class="bke-hint">${hint}</div>
            <div class="bke-actions">
                <button class="bke-btn-retry" id="bkeBtnRetry" onclick="hideBookingError(); document.getElementById('submitBtn').click();">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                        <polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 1 0 .49-3.51"/>
                    </svg>
                    Coba Lagi
                </button>
                <button class="bke-btn-dismiss" onclick="hideBookingError()">Tutup</button>
            </div>
        </div>
    `;
    ...
    // Scroll banner ke viewport di dalam modal
    setTimeout(() => {
