...
let blockedSeats         = new Set(); // kursi dengan status='blocked' di DB (ditampilkan abu-abu)
...
function createSeatElement(section, row, seatNum, zoneClass) {
    ...
    if (individual) {
        ...
        if (window.seatStatus?.[zone]?.closed === true) {
            ...
            // Kursi diblokir manual oleh admin (status='blocked' di DB) — tampil abu-abu
            seatDiv.classList.add('blocked-seat');
            seatDiv.style.pointerEvents = 'none';
            seatDiv.title = `${getSeatLabel(zone)} — ${section === 'L' ? 'Kiri' : 'Kanan'} Baris ${row} No ${seatNum} — Tidak tersedia`;
...
function buildCardHTML(type, section, available, isSelected, selCount, color, label, isClosed = false) {
    if (isClosed) {
        ...
        <div class="zoc-inner" style="opacity:.6;filter:grayscale(.5);">
            ...
            <div class="zoc-avail">
                <span class="zoc-count" style="color:${color};">0</span>
    ...
    // ── HABIS TERJUAL: available === 0 ──────────────────────────────
    if (available <= 0) {
        return `
        <div class="zoc-inner" style="opacity:.65;filter:grayscale(.4);">
            <div class="zoc-top-row">
                <span class="zoc-pill" style="border-color:${color}66;">${sectionLabels[section]}</span>
                <span style="font-size:10px;font-weight:700;letter-spacing:.4px;background:#fef2f2;color:#991b1b;border:1px solid #fecaca;border-radius:4px;padding:2px 7px;">HABIS TERJUAL</span>
            </div>
            <div class="zoc-name" style="color:${color};">${label}</div>
            <div class="zoc-sub">${sectionNames[section]}</div>
            <div class="zoc-sep" style="background:${color}44;"></div>
            <div class="zoc-avail">
                <span class="zoc-count" style="color:#ef4444;">0</span>
                <span class="zoc-label">kursi tersedia</span>
            </div>
            <div class="zoc-cta" style="border-color:#fecaca;color:#991b1b;background:#fef2f2;cursor:not-allowed;">
                &#10060; Habis Terjual
            </div>
        </div>
        `;
...
function refreshAllOverlayCards() {
    ...
    // Refresh juga zone badge VVIP/VIP (karena tergantung bookedSeats yang baru terisi)
    refreshZoneBadges();
}

function refreshZoneBadges() {
    const cfg = window.ticketConfig || [];
    const individualZones = cfg.slice(0, 2).map(t => t.type); // VVIP & VIP
    const zoneToClass = {};
    if (individualZones[0]) zoneToClass[individualZones[0]] = 'vvip-zone-label';
    if (individualZones[1]) zoneToClass[individualZones[1]] = 'vip-zone-label';
    Object.entries(zoneToClass).forEach(([zone, cls]) => {
        document.querySelectorAll('.' + cls).forEach(el => {
            el.innerHTML = _buildZoneBadgeHTML(zone);
        });
    });
}
...
function handleSeatClick(seatId) {
    ...
    if (!seatDiv || seatDiv.classList.contains('sold') || seatDiv.classList.contains('blocked-seat')) return;
...
function showTicketCountModal(type, section) {
    ...
    // Blokir jika kursi habis
    if (available <= 0) {
        showWarning('Kursi Habis Terjual', `Maaf, kursi ${getSeatLabel(type)} ${sectionNames[section]} sudah habis terjual.`);
        return;
    }
...
function getAvailableSeatsForSection(type, section) {
    ...
    // Zona individual (VVIP/VIP): pakai bookedSeats + blockedSeats (diblokir manual)
    return totalSeats.filter(seatId => !bookedSeats.has(seatId) && !blockedSeats.has(seatId));
}
...
function _buildZoneBadgeHTML(zone) {
    ...
    // Cek apakah semua kursi zona ini sudah habis (setelah bookedSeats terisi)
    const availL = getAvailableSeatsForSection(zone, 'L').length;
    const availR = getAvailableSeatsForSection(zone, 'R').length;
    if (availL + availR <= 0) {
        return `<span class="zone-badge-btn zone-badge-closed" style="--zb-color:#ef4444;--zb-bg:#fef2f2;">
                    — ${label} Zone — <span class="zb-tag" style="background:#ef4444;color:#fff;">Habis Terjual</span>
                </span>`;
    }
...
window.openZoneModal = function(zone) {
    ...
    // Hitung sisa kursi yang benar-benar tersedia (total fisik dikurangi yang sudah sold/booked)
    const availL = getAvailableSeatsForSection(zone, 'L').length;
    const availR = getAvailableSeatsForSection(zone, 'R').length;
    const availTotal = availL + availR;

    // Blokir jika habis
    if (availTotal <= 0) {
        showWarning('Kursi Habis Terjual', `Maaf, semua kursi zona ${label} sudah habis terjual.`);
        return;
    }

    // quota yang bisa dipilih = sisa kursi yang tersedia (bukan open_quota awal)
    const quota = availTotal;
...
function _enableSeatPickerClicks(zone, color, total) {
    ...
    document.querySelectorAll('.seat.vvip-vip-locked').forEach(seat => {
        ...
        if (seat.classList.contains('sold') || seat.classList.contains('archived-zone') || seat.classList.contains('blocked-seat')) return;
