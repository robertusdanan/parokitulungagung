...
if (!function_exists('privatePath')) {
    function privatePath(string $subpath = ''): string {
        $subpath = ltrim($subpath, '/\\');
        $docRoot = $_SERVER['DOCUMENT_ROOT'] ?? '';
        if ($docRoot !== '') {
            $parent = dirname(rtrim($docRoot, '/\\'));
            $candidate = $parent . '/private' . ($subpath !== '' ? '/' . $subpath : '');
            if (file_exists($candidate) || is_dir(dirname($candidate))) return $candidate;
        }
        $dir = __DIR__;
        while ($dir && $dir !== '/' && $dir !== '.') {
            $candidate = $dir . '/private' . ($subpath !== '' ? '/' . $subpath : '');
            if (file_exists($candidate) || is_dir(dirname($candidate))) return $candidate;
            $parent = dirname($dir);
            if ($parent === $dir) break;
            $dir = $parent;
        }
        return '/workspace/private' . ($subpath !== '' ? '/' . $subpath : '');
    }
}
