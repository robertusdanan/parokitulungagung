...
/* ── Tombol ganti foto ── */
.btn-photo-sm { background: #fef3c7; color: #92400e; }

/* ── Section foto di dalam edit modal ── */
.edit-photo-section {
    border: 1.5px solid var(--border);
    border-radius: 14px;
    padding: 16px;
...
.edit-photo-dropzone:hover, .edit-photo-dropzone.dragover {
    border-color: var(--primary);
    background: #eef2ff;
}
.edit-photo-dropzone input[type="file"] {
    position: absolute;
    inset: 0;
    opacity: 0;
    cursor: pointer;
...
.edit-photo-dropzone p { font-size: .82rem; color: var(--gray); margin: 0; }
.edit-photo-dropzone p strong { color: var(--primary); }
.edit-photo-new-preview {
    display: none;
    position: relative;
    margin-top: 8px;
    border-radius: 8px;
...
.edit-photo-new-preview img {
    width: 100%;
    max-height: 120px;
    object-fit: cover;
    display: block;
}
.edit-photo-new-preview-remove {
    position: absolute;
    top: 6px; right: 6px;
...
.edit-photo-new-preview-remove:hover { background: rgba(220,38,38,.8); }
.edit-photo-progress {
    display: none;
    margin-top: 8px;
    height: 5px;
...
<div class="edit-modal-overlay" id="editModalOverlay" onclick="handleEditOverlayClick(event)">
    <div class="edit-modal-box">
        ...
        <!-- ── SECTION GANTI FOTO BUKTI PEMBAYARAN ── -->
        <div class="edit-photo-section">
            <div class="edit-photo-section-label">🖼️ Foto Bukti Pembayaran</div>

            <!-- Foto saat ini -->
            <div id="editCurrentPhotoWrap">
                <!-- Diisi oleh JS saat modal dibuka -->
            </div>

            <!-- Dropzone upload baru -->
            <div class="edit-photo-dropzone" id="editPhotoDropzone">
                <input type="file" id="editPhotoFileInput" accept="image/jpeg,image/png,image/webp,image/gif,image/heic" onchange="onEditPhotoSelected(event)">
                <p>📤 <strong>Klik atau seret foto baru ke sini</strong></p>
                <p style="margin-top:3px;font-size:.75rem;color:var(--gray-light);">JPG, PNG, WEBP, HEIC — maks. 10MB</p>
            </div>

            <!-- Preview foto baru -->
            <div class="edit-photo-new-preview" id="editPhotoNewPreview">
                <img id="editPhotoNewPreviewImg" src="" alt="Preview">
                <button class="edit-photo-new-preview-remove" onclick="removeEditPhotoPreview()" title="Batal pilih">✕</button>
            </div>

            <!-- Progress & status -->
            <div class="edit-photo-progress" id="editPhotoProgress">
                <div class="edit-photo-progress-bar" id="editPhotoProgressBar"></div>
            </div>
            <p id="editPhotoStatus" style="font-size:.78rem;color:var(--gray);margin-top:6px;min-height:16px;"></p>
        </div>
...
let editPhotoSelectedFile = null; // foto baru yang dipilih di edit modal
...
// Normalisasi URL foto — pastikan selalu mengacu ke /e-ticket/uploads/payment-proofs/
function resolvePhotoUrl(url) {
    if (!url) return '';
    // Sudah URL absolut (http/https) → pakai langsung
    if (/^https?:\/\//i.test(url)) return url;
    // Sudah mengandung /e-ticket/ → pakai langsung
    if (url.indexOf('/e-ticket/') !== -1) return url;
    // Hanya nama file (tanpa slash di depan) → tambah prefix lengkap
    if (!url.startsWith('/')) return '/e-ticket/uploads/payment-proofs/' + url;
    // Path dimulai /uploads/... → tambah /e-ticket di depan
    if (url.startsWith('/uploads/')) return '/e-ticket' + url;
    // Fallback
    return url;
}
...
function openEditModal(ticketId) {
    ...
    // ── Reset & isi section foto ──
    editPhotoSelectedFile = null;
    document.getElementById('editPhotoFileInput').value = '';
    document.getElementById('editPhotoNewPreview').style.display = 'none';
    document.getElementById('editPhotoNewPreviewImg').src = '';
    document.getElementById('editPhotoProgress').style.display = 'none';
    document.getElementById('editPhotoProgressBar').style.width = '0%';
    ...
    if (t.image_url && t.image_url.trim() !== '') {
        const photoUrl = resolvePhotoUrl(t.image_url);
        currentWrap.innerHTML = `
            <div style="margin-bottom:8px;">
                <p style="font-size:.75rem;font-weight:600;color:var(--gray);margin-bottom:5px;text-transform:uppercase;letter-spacing:.4px;">📷 Foto Saat Ini</p>
                <img class="edit-photo-current" src="${photoUrl}" alt="Bukti pembayaran"
                     onerror="this.parentElement.innerHTML='<div class=\\'edit-photo-no-img\\'>⚠️ Foto tidak dapat dimuat</div>'">
            </div>`;
    } else {
        currentWrap.innerHTML = `<div class="edit-photo-no-img">📭 Belum ada foto bukti pembayaran</div>`;
    }
...
async function saveEditTicket() {
    ...
    try {
        ...
        if (editPhotoSelectedFile) {
            btn.textContent = '⏳ Upload foto...';
            const statusEl  = document.getElementById('editPhotoStatus');
            const progWrap  = document.getElementById('editPhotoProgress');
            const progBar   = document.getElementById('editPhotoProgressBar');
            progWrap.style.display = 'block';
            progBar.style.width    = '30%';
            statusEl.textContent   = '📤 Mengunggah foto...';

            const formData = new FormData();
            formData.append('image', editPhotoSelectedFile);
            formData.append('ticket_number', ticketNumber);

            const uploadResp = await fetch('/e-ticket/upload.php', {
                method: 'POST',
                body:   formData,
            });
            ...
            if (!uploadResp.ok) {
                ...
                throw new Error('Upload foto gagal: ' + (errData.error || `HTTP ${uploadResp.status}`));
            }
            const uploadData = await uploadResp.json();
            if (!uploadData.success) throw new Error('Upload foto gagal: ' + (uploadData.error || 'Error tidak diketahui'));

            patch.image_url        = uploadData.url;
        ...
        showToast(editPhotoSelectedFile ? '✅ Tiket & foto berhasil diupdate!' : '✅ Tiket berhasil diupdate!', 'success');
        closeEditModal();
        renderTicketList();

    } catch(err) {
        showToast('❌ Gagal update: ' + err.message, 'error');
        document.getElementById('editPhotoStatus').textContent = '❌ ' + err.message;
    } finally {
        btn.disabled    = false;
...
// ────────────────────────────────────────────────────────────
// FOTO BUKTI BAYAR — handler di dalam edit modal
// ────────────────────────────────────────────────────────────
function onEditPhotoSelected(event) {
    ...
    editPhotoSelectedFile = file;
    document.getElementById('editPhotoStatus').textContent =
        `📁 ${file.name} (${(file.size / 1024).toFixed(0)} KB) — akan diupload saat Simpan`;

    const reader = new FileReader();
    reader.onload = function(e) {
        const wrap = document.getElementById('editPhotoNewPreview');
...
function removeEditPhotoPreview() {
    editPhotoSelectedFile = null;
    document.getElementById('editPhotoFileInput').value = '';
    document.getElementById('editPhotoNewPreview').style.display = 'none';
    document.getElementById('editPhotoNewPreviewImg').src = '';
...
// Drag & drop di edit modal dropzone
document.addEventListener('DOMContentLoaded', function() {
    const dz = document.getElementById('editPhotoDropzone');
    if (!dz) return;
    ...
    dz.addEventListener('drop', function(e) {
        ...
        if (file) {
            ...
            onEditPhotoSelected({ target: input });
        }
    });
});
