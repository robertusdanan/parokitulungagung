...
/**
 * Validasi URL "kembali ke halaman ini" (dipakai oleh login, register, logout)
 * supaya HANYA path internal situs sendiri yang boleh dituju — mencegah
 * open-redirect ke domain luar lewat parameter ?redirect=.
 */
function safeRedirectBack(?string $url): ?string
{
    if (!$url) return null;
    $url = trim($url);
    if ($url === '') return null;
    // Harus mulai dengan "/" tunggal (bukan "//" atau "/\" yang bisa dianggap
    // browser sebagai protocol-relative URL ke domain lain), dan tidak
    // mengandung skema (http://, https://) di bagian manapun.
    if (!preg_match('#^/(?!/)(?!\\\\)[^\s]*$#', $url)) return null;
    if (stripos($url, '://') !== false) return null;
    return $url;
}
...
function startAdminSession(): void
{
    if (session_status() === PHP_SESSION_NONE) {
        ...
        session_set_cookie_params([
            ...
            // Path '/' (bukan '/admin') supaya halaman publik (Home, Artikel, dll)
            // ikut menerima cookie ini dan bisa mendeteksi status login admin
            // untuk menampilkan avatar + dropdown di header. Cookie tetap
            // httponly + secure + SameSite=Strict, jadi tidak ada perubahan
            // pada tingkat keamanan — hanya cakupan pengiriman cookie-nya.
            'path'     => '/',
        ...
        // ── Bersihkan cookie lama (path=/admin) dari skema sebelum update ──
        // Browser bisa menyimpan 2 cookie dengan nama sama tapi path beda
        // (/admin dan /) sekaligus, dan itu bisa membuat PHP salah membaca
        // cookie yang mana yang dipakai — efeknya login terasa "gagal terus"
        // padahal sebenarnya session-nya "terpecah". Expire cookie lama ini
        // secara eksplisit supaya hanya ada 1 cookie yang tersisa.
