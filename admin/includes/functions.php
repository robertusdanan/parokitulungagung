...
// `e()` juga didefinisikan di includes/functions.php (helper publik). Jangan
// redeclare di sini — admin/page akan load helper publik & admin sekaligus.
...
function adminHeader(string $pageTitle, string $activePage, array $user): void {
    ...
    // Data pages = semua kecuali artikel, master, dan media (media masuk ke Sistem), serta submenu agenda
    $excludeFromData = array_merge(ARTIKEL_PAGES, ['master', 'media', 'dokumen_paroki', 'jadwal_petugas_gambar']);
...
<head>
  ...
  <script>
    ...
    window.PERSON_CDN_URL = <?= json_encode(rtrim(R2_CDN_URL, '/')) ?> + '/person';
    window.ICON_CDN_URL   = <?= json_encode(rtrim(R2_CDN_URL, '/')) ?>;
    window.ASSETS_CDN_URL = <?= json_encode(rtrim(R2_CDN_URL, '/')) ?> + '/assets';
    // URL spesifik ikon Romo (fallback foto profil di beberapa halaman admin).
    // Dibangun manual (tanpa helper iconUrl() publik) karena helper itu didefinisikan
    // di includes/functions.php yang tidak selalu di-load oleh halaman admin.
    window.ICON_ROMO_URL  = <?= json_encode(rtrim(R2_CDN_URL, '/') . '/icon/icon_romo.png') ?>;
...
<div class="admin-layout">
  <aside class="sidebar" id="sidebar">
    <div class="sidebar-brand">
      <img class="brand-logo" src="https://img.parokitulungagung.org/assets/parokitulungagung.webp" alt="Logo Paroki" width="34" height="34">
      <div>
    ...
    <nav class="sidebar-nav">
      ...
      $dataIcons = [
          ...
          'stories'      => '<circle cx="12" cy="12" r="10"/><polygon points="10 8 16 12 10 16 10 8"/>',
          ...
          'romo_paroki'  => '<path d="M12 2v6M9 5h6"/><circle cx="12" cy="13" r="7"/><path d="M12 10v6M9 13h6"/>',
      ...
      <a href="/admin/pages/cache.php" class="nav-item <?= $activePage === 'cache' ? 'active' : '' ?>">
  ...
  <div class="main-area">
    <header class="topbar">
      ...
      <div class="topbar-right">
        <a href="/" class="btn-view-site" title="Kembali ke Homepage Website">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="15" height="15" aria-hidden="true"><path d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
...
// ── Cache Helpers ──────────────────────────────────────────────────────
// Lihat admin/includes/CacheRegistry.php — semua grup cache (supabase,
// runtime, galeri, r2img, analytics, imgseo, sitemap, media, chatbot)
// sekarang dikelola dari satu tempat, dipakai oleh admin/pages/cache.php
// dan admin/api/cache.php.