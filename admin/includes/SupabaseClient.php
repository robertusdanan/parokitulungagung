...
class SupabaseClient
{
     ...
     *
     * SECURITY FIX: filter di sini biasanya dibangun oleh caller dengan
     * menempelkan input user langsung, mis. 'username=ilike.' . $username
     * (lihat UserManager::findByUsername, dipanggil dari form login publik;
     * dan reset-password.php dengan $token dari link publik). Sebelum fix
     * ini, nilainya TIDAK di-encode sama sekali sebelum masuk ke query
     * string, jadi karakter seperti & , ( ) % bisa dipakai untuk menyisipkan
     * parameter/filter PostgREST tambahan (query/filter injection) — atau
     * memakai wildcard ilike (%) untuk mencocokkan baris yang tidak seharusnya
     * bisa ditarget. Sekarang bagian VALUE tiap filter selalu di-encode,
     * sementara bagian "kolom=operator." dibiarkan apa adanya.
     */
    public function readWhere(string $table, array $rawFilters = [], string $order = '', string $select = '*'): array
    {
        ...
        $safeFilters = array_map(function (string $filter): string {
            // Pisah di titik PERTAMA: "kolom=operator" | "value"
            // (nama kolom & operator PostgREST — eq, ilike, gt, dst — tidak
            // pernah mengandung titik, jadi titik pertama selalu batas value)
            $parts = explode('.', $filter, 2);
            if (count($parts) !== 2) {
                // Tidak dalam format "kolom=operator.value" yang dikenal —
                // jangan diloloskan mentah-mentah, encode seluruhnya agar aman.
                return rawurlencode($filter);
            }
            [$prefix, $value] = $parts;
            return $prefix . '.' . rawurlencode($value);
        }, $rawFilters);

        $queryStr = http_build_query($params);
        if ($safeFilters) {
            $queryStr .= '&' . implode('&', $safeFilters);
        }
