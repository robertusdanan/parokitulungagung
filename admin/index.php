...
// ── Redirect balik ke halaman terakhir (Home/Artikel/dll) setelah login ──
// Diisi dari tombol "Login" di header publik, atau diteruskan dari
// register.php setelah pendaftaran berhasil.
$redirectTo = safeRedirectBack($_POST['redirect'] ?? $_GET['redirect'] ?? null);

if (!empty($_SESSION['admin_user'])) {
    header('Location: ' . ($redirectTo ?: '/admin/dashboard.php'));
    exit;
}
  ...
  <div class="login-box">
    ...
    <form class="login-form" method="POST" autocomplete="on">
      ...
      <div class="cf-turnstile"
           data-sitekey="0x4AAAAAAECePtEX2GmxUA8F"
           data-theme="dark"
      ...
      <button type="submit" class="btn btn-primary login-btn" id="loginSubmitBtn" disabled>
        <span id="loginSubmitLabel">Menunggu verifikasi…</span>
      </button>
    ...
    <a href="/admin/register.php<?= $redirectTo ? '?redirect=' . urlencode($redirectTo) : '' ?>" class="btn btn-secondary"
    ...
    <div style="text-align:center;margin-top:14px">
      <a href="<?= $redirectTo ? e($redirectTo) : '/' ?>" style="font-size:12px;color:var(--text-muted);">← Kembali ke Website</a>
    </div>
  </div>
</div>
<script>
...
/* ============================================================
   CLOUDFLARE TURNSTILE — kontrol tombol "Masuk"
   - Loading / belum diverifikasi → tombol disabled
   - Berhasil (centang hijau)      → tombol enabled, submit normal
   - Gagal / expired / timeout     → tombol enabled tapi klik = reload halaman
   ============================================================ */
var turnstileState = 'loading'; // 'loading' | 'success' | 'error'
...
function setTurnstileState(state) {
  ...
  if (state === 'success') {
    ...
    loginBtn.disabled = false;
    loginLabel.textContent = 'Coba Lagi';
  } else {
    loginBtn.disabled = true;
    loginLabel.textContent = 'Menunggu verifikasi…';
  }
}

window.onTurnstileSuccess = function () { setTurnstileState('success'); };
window.onTurnstileError   = function () { setTurnstileState('error'); };
window.onTurnstileExpired = function () { setTurnstileState('loading'); };
...
loginBtn.addEventListener('click', function (e) {
  ...
  // state 'loading' -> tombol memang disabled, tidak akan ter-klik
  // state 'success' -> biarkan submit form berjalan normal
});
</script>
</body>
</html>