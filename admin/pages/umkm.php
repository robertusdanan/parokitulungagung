...
function renderGrid(data) {
  ...
  grid.innerHTML = sorted.map(r => {
    ...
    const imgSrc   = r['gambar'] ? (r['gambar'].startsWith('http') ? r['gambar'] : cdnBase + '/umkm/' + r['gambar']) : '';
  ...
  initPagination('umkmGrid', 20, ':scope > .umkm-card');
}
...
function openEditModal(id) {
  ...
  if (r['gambar']) {
    ...
    document.getElementById('previewImg').src            = r['gambar'].startsWith('http') ? r['gambar'] : cdnBase + '/umkm/' + r['gambar'];
