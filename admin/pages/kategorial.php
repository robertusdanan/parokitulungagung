 ...
 */
require_once __DIR__ . '/../../includes/functions.php';   // helper publik (iconUrl, iconKategorialUrl, personPhotoUrl, dll.)
require_once __DIR__ . '/../includes/functions.php';     // helper admin (adminHeader, getDB, e(), dll.)
adminBoot();
$user = requireLogin();
...
$BUILT_IN_ICONS = [
    ...
    'pdkk'           => iconKategorialUrl('pdkk.png'),
    'wanita-katolik' => iconKategorialUrl('wanita-katolik.png'),
    'gim'            => iconKategorialUrl('gim.png'),
    ...
    'ssvrosali'      => iconKategorialUrl('ssvrosali.png'),
];
    ...
    if (is_array($rows)) {
        foreach ($rows as $r) {
            if (is_array($r) && !empty($r['slug'])) {
                if (!empty($r['icon'])) {
                    $r['icon'] = iconKategorialUrl($r['icon']);
                }
                $allProfil[$r['slug']] = $r;
            }
        }
...
<div id="viewCard" class="card" style="padding:20px">
  <div class="kat-grid">
    <?php foreach ($allSlugs as $slug):
      ...
      $icon        = !empty($p['icon']) ? iconKategorialUrl($p['icon']) : ($BUILT_IN_ICONS[$slug] ?? '');
    ...
    <div class="kat-card" onclick="<?= $canEdit ? "openEditModal('".e($slug)."')" : "window.open('/kategorial/".urlencode($slug)."','_blank')" ?>">
      ...
      <?php endif; ?>
      <?php if ($icon): ?>
        <img src="<?= e($icon) ?>" alt="" class="kat-card-icon" onerror="this.style.display='none';if(this.nextElementSibling)this.nextElementSibling.style.display='flex'">
          ...
          <div class="form-group full-col">
            ...
            <div class="img-picker" id="pickerBanner" onclick="triggerPicker('banner')">
              <div class="img-picker-preview" id="previewBanner">
                ...
                <img id="imgPreviewBanner" src="" alt="" style="display:none;width:100%;height:100%;object-fit:cover;border-radius:6px" onerror="this.style.display='none';document.getElementById('emptyBanner').style.display='flex'">
              </div>
            ...
            <div style="display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap">
              <div class="img-picker img-picker--icon" id="pickerIcon" onclick="triggerPicker('icon')">
                <div class="img-picker-preview img-picker-preview--icon" id="previewIcon">
                  ...
                  <img id="imgPreviewIcon" src="" alt="" style="display:none;width:100%;height:100%;object-fit:contain;padding:6px;border-radius:50%" onerror="this.style.display='none';document.getElementById('emptyIcon').style.display='flex'">
                </div>
              </div>
...
async function uploadImage(file, type) {
  ...
  try {
    const form=new FormData(); form.append('file',file); form.append('folder',type==='banner'?'banner':'icon_kategorial');
    const resp=await fetch('/admin/api/upload_image.php',{method:'POST',body:form,credentials:'same-origin'});
    const data=await resp.json();
    if(!data.success){toast('Error',data.error||'Upload gagal','error');return;}
...
function resolveKatIconUrl(icon) {
  if (!icon) return '';
  if (icon.startsWith('http://') || icon.startsWith('https://')) return icon;
  const clean = icon.replace(/^\/?(img\/)?(icon\/(kategorial\/)?)?/i, '');
  const base = (window.ICON_CDN_URL || 'https://img.parokitulungagung.org').replace(/\/$/, '');
  return base + '/icon/kategorial/' + clean.replace(/^\//, '');
}
...
function loadImagePreviews(banner,icon) {
  ...
  if(icon){
    const resolved = resolveKatIconUrl(icon);
    document.getElementById('fieldIcon').value=resolved;
    const img=document.getElementById('imgPreviewIcon');
    img.src=resolved; img.style.display='block';
...
async function loadMediaGallery(page=1) {
  ...
  try {
    // Tentukan folder: banner, person, atau icon_kategorial
    const folder = _mediaCurrent==='banner' ? 'banner' : (_mediaCurrent==='person' ? 'person' : 'icon_kategorial');
...
async function mediaUploadOne(file) {
  ...
  try {
    const form=new FormData(); form.append('file',file); form.append('folder',_mediaCurrent==='banner'?'banner':'icon_kategorial');
    const resp=await fetch('/admin/api/upload_image.php',{method:'POST',body:form,credentials:'same-origin'});
    const data=await resp.json();
