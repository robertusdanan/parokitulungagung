...
$hasAgenda  = $isSuper || array_key_exists('agenda', $userPerms);
$hasDokumen = $isSuper || array_key_exists('dokumen_paroki', $userPerms);
$hasPetugas = $isSuper || array_key_exists('jadwal_petugas_gambar', $userPerms);

if (!$hasAgenda && !$hasDokumen && !$hasPetugas) {
...
$canCreateAgenda  = $isSuper || in_array('create', $userPerms['agenda'] ?? []);
$canCreateDokumen = $isSuper || in_array('create', $userPerms['dokumen_paroki'] ?? []);
$canCreatePetugas = $isSuper || in_array('create', $userPerms['jadwal_petugas_gambar'] ?? []);
...
<div class="page-header">
  ...
  <div id="headerActionWrap">
    <?php if ($activeTab === 'agenda' && $canCreateAgenda): ?>
    <button class="btn btn-primary" id="btnTambahUtama" onclick="openAddAgendaModal()">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
      Tambah Agenda
    </button>
    <?php elseif ($activeTab === 'dokumen' && $canCreateDokumen): ?>
    <button class="btn btn-primary" id="btnTambahUtama" onclick="openAddDokumenModal()">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
      Upload Dokumen
    </button>
    <?php elseif ($activeTab === 'petugas' && $canCreatePetugas): ?>
    <button class="btn btn-primary" id="btnTambahUtama" onclick="openAddPetugasModal()" style="background:linear-gradient(135deg,#007070,#009090)">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
      Upload Jadwal
    </button>
    <?php endif; ?>
...
<div class="admin-master-tabs" id="adminMasterTabs">
  <?php if ($hasAgenda): ?>
  <button class="amt-tab <?= $activeTab === 'agenda' ? 'amt-tab--active' : '' ?>" onclick="switchMasterTab('agenda',this)" id="tabBtnAgenda">
  ...
  <?php endif; ?>
  <?php if ($hasDokumen): ?>
  <button class="amt-tab <?= $activeTab === 'dokumen' ? 'amt-tab--active' : '' ?>" onclick="switchMasterTab('dokumen',this)" id="tabBtnDokumen">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="15" height="15">
  ...
  <?php endif; ?>
  <?php if ($hasPetugas): ?>
  <button class="amt-tab <?= $activeTab === 'petugas' ? 'amt-tab--active' : '' ?>" onclick="switchMasterTab('petugas',this)" id="tabBtnPetugas">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" width="15" height="15">
...
/* 1 item: tidak menyempit di pojok kiri, melainkan melebar proporsional */
.petugas-admin-grid > .petugas-admin-card:only-child {
  max-width: 360px;
  width: 100%;
  justify-self: start;
}
...
.petugas-admin-img:hover { transform: scale(1.02); }
.petugas-admin-body {
  padding: 12px 14px;
...
.petugas-admin-actions .btn { flex: 1; }
...
/* ── Lightbox preview Jadwal Petugas ── */
.petugas-lightbox {
...
.petugas-lightbox--open { opacity: 1; visibility: visible; }
.plb-backdrop {
...
.petugas-lightbox--open .plb-wrap { transform: scale(1); }
.plb-close {
...
.plb-close:hover { background: rgba(0,0,0,0.85); transform: rotate(90deg); }
.plb-title {
  padding: 14px 20px;
...
.plb-btn:hover { border-color: var(--accent, #b8860b); color: var(--accent, #b8860b); }
.plb-btn--primary {
  background: var(--accent, #b8860b);
  color: #fff;
  border-color: var(--accent, #b8860b);
}
.plb-btn--primary:hover { background: #a0750a; color: #fff; border-color: #a0750a; }

@media (max-width: 600px) {
  .petugas-lightbox { padding: 0; }
  .plb-wrap { max-width: 100vw; max-height: 100vh; border-radius: 0; }
  .plb-img { max-height: 75vh; }
}
</style>
...
<!-- ═══════════════════════════════════════════════════════════════════
     PANEL: AGENDA
═══════════════════════════════════════════════════════════════════ -->
<div id="panel-agenda" style="<?= $activeTab === 'agenda' ? '' : 'display:none' ?>">
...
═══════════════════════════════════════════════════════════════════ -->
<div id="panel-dokumen" style="<?= $activeTab === 'dokumen' ? '' : 'display:none' ?>">
<div class="card" style="border-radius:0 12px 12px 12px;">
  <div class="toolbar">
    <div class="toolbar-left">
      <div class="search-wrap">
...
═══════════════════════════════════════════════════════════════════ -->
<div id="panel-petugas" style="<?= $activeTab === 'petugas' ? '' : 'display:none' ?>">
<div class="card" style="border-radius:0 12px 12px 12px;">
  <div class="toolbar">
    <div class="toolbar-left">
      <div class="search-wrap">
  ...
  <div id="petugasTableContainer" style="display:none">
    <div class="table-wrapper">
...
═══════════════════════════════════════════════════════════════════ -->
<?php
$canEditAgenda  = $isSuper || in_array('edit', $userPerms['agenda'] ?? []);
$canEditDokumen = $isSuper || in_array('edit', $userPerms['dokumen_paroki'] ?? []);
$canEditPetugas = $isSuper || in_array('edit', $userPerms['jadwal_petugas_gambar'] ?? []);

$canCreateOrEditAny = $canCreateAgenda || $canCreateDokumen || $canCreatePetugas ||
                      $canEditAgenda || $canEditDokumen || $canEditPetugas;

if ($canCreateOrEditAny):
?>
...
═══════════════════════════════════════════════════════════════════ -->
<script>
// Redefine window.can to support specific page action checking
window.PERMISSIONS = <?= json_encode(getPermissionsMap($user)) ?>;
window.isSuperadmin = <?= $isSuper ? 'true' : 'false' ?>;

window.can = function(action, page = 'agenda') {
    if (window.isSuperadmin) return true;
    const actions = window.PERMISSIONS[page] || [];
    if (action === 'view' || action === 'list') {
        return window.PERMISSIONS.hasOwnProperty(page);
    }
...
function switchMasterTab(name, btn) {
    ...
    if (btn) btn.classList.add('amt-tab--active');
    ...
    const canCreateAgenda  = <?= $canCreateAgenda ? 'true' : 'false' ?>;
    const canCreateDokumen = <?= $canCreateDokumen ? 'true' : 'false' ?>;
    const canCreatePetugas = <?= $canCreatePetugas ? 'true' : 'false' ?>;

    if (name === 'agenda') {
        haw.innerHTML = canCreateAgenda
        ...
        haw.innerHTML = canCreateDokumen
        ...
        haw.innerHTML = canCreatePetugas
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!can('edit', 'agenda') && !can('delete', 'agenda')) {
...
function renderTable(data) {
    ...
    document.getElementById('emptyState').style.display = 'none';
    const showAksi = can('edit', 'agenda') || can('delete', 'agenda');
    tbody.innerHTML = data.map(row => {
        ...
        return `<tr ${isLibur ? 'style="background:rgba(224,82,82,0.04)"' : ''}>
          ...
          ${showAksi?`<td><div class="actions">
            ${can('edit', 'agenda')?`<button class="btn btn-icon btn-sm" onclick="openEditAgendaModal(${row._id})">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>`:''}
            ${can('delete', 'agenda')?`<button class="btn btn-icon btn-sm" onclick="deleteAgendaRow(${row._id})" style="color:var(--danger);border-color:rgba(224,82,82,0.3)">
...
function openAddAgendaModal() {
    if (!can('create', 'agenda')) { toast('Akses Ditolak','Tidak ada izin tambah','error'); return; }
    editAgendaRow = null;
...
function openEditAgendaModal(rowNum) {
    if (!can('edit', 'agenda')) { toast('Akses Ditolak','Tidak ada izin edit','error'); return; }
...
async function submitAgendaForm() {
    ...
    if (isEdit && !can('edit', 'agenda'))    { toast('Akses Ditolak','Tidak ada izin edit','error'); return; }
    if (!isEdit && !can('create', 'agenda')) { toast('Akses Ditolak','Tidak ada izin tambah','error'); return; }
...
async function deleteAgendaRow(rowNum) {
    if (!can('delete', 'agenda')) { toast('Akses Ditolak','Tidak ada izin hapus','error'); return; }
...
function renderDokumenTable(data) {
    ...
    tbody.innerHTML = data.map((dk) => {
        ...
        return `<tr>
          ...
          <td><div class="actions">
            ${can('edit', 'dokumen_paroki')?`<button class="btn btn-icon btn-sm" onclick="openEditDokumenModal(${dk._id})" title="Edit">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>`:''}
            <a href="<?= defined('R2_CDN_URL') ? rtrim(R2_CDN_URL, '/') : 'https://img.parokitulungagung.org' ?>/downloads/${encodeURIComponent(namaFile)}" target="_blank"
               class="btn btn-icon btn-sm" title="Unduh" style="color:var(--accent)">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            </a>
            ${can('delete', 'dokumen_paroki')?`<button class="btn btn-icon btn-sm" onclick="deleteDokumen(${dk._id})" title="Hapus" style="color:var(--danger);border-color:rgba(224,82,82,0.3)">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>
            </button>`:''}
          </div></td>
        </tr>`;
    ...
    initPagination('dokumenTable', 20);
}
...
function openAddDokumenModal() {
    if (!can('create', 'dokumen_paroki')) { toast('Akses Ditolak','Tidak ada izin upload','error'); return; }
...
function openEditDokumenModal(id) {
    if (!can('edit', 'dokumen_paroki')) { toast('Akses Ditolak','Tidak ada izin edit','error'); return; }
...
async function submitDokumenForm() {
    ...
    if (isEdit && !can('edit', 'dokumen_paroki'))    { toast('Akses Ditolak','Tidak ada izin edit','error'); return; }
    if (!isEdit && !can('create', 'dokumen_paroki')) { toast('Akses Ditolak','Tidak ada izin upload','error'); return; }
...
async function deleteDokumen(id) {
    if (!can('delete', 'dokumen_paroki')) { toast('Akses Ditolak','Tidak ada izin hapus','error'); return; }
...
async function loadPetugas() {
    ...
    document.getElementById('petugasTableContainer').style.display = 'none';
...
function renderPetugasTable(data) {
    ...
    document.getElementById('petugasTableContainer').style.display = 'block';
    document.getElementById('petugasEmptyState').style.display = 'none';

    const cdnBase = '<?= defined("R2_CDN_URL") ? rtrim(R2_CDN_URL, "/") : "https://img.parokitulungagung.org" ?>';
    tbody.innerHTML = data.map(p => {
        const imgUrl   = cdnBase + '/jadwal_petugas/' + encodeURIComponent(p.nama_file || '');
        const isAktif  = p.aktif === true || p.aktif === 'true' || p.aktif === 1;
        const ext      = ((p.nama_file||'').split('.').pop()||'img').toUpperCase();
        return `<tr>
          <td style="text-align:center;color:var(--text-secondary);font-size:12px">${p.urutan||0}</td>
          <td>
            <div style="font-weight:600;font-size:13.5px;color:var(--text-primary)">${escHtml(p.judul||'Jadwal Petugas')}</div>
            <div style="font-size:11.5px;color:var(--text-secondary);margin-top:2px">Jadwal Misa &amp; Petugas Liturgi</div>
          </td>
          <td>
            <div class="dt-icon-cell">
              <span class="dt-file-badge" style="background:#008080">${escHtml(ext)}</span>
              <div style="font-size:12px;color:var(--text-secondary);word-break:break-all">${escHtml(p.nama_file||'')}</div>
            </div>
          </td>
          <td>
            <div style="width:48px;height:48px;border-radius:6px;overflow:hidden;border:1px solid var(--border);cursor:zoom-in;background:#151c24;position:relative;display:inline-flex;align-items:center;justify-content:center"
                 onclick="openPetugasLightbox('${escHtml(imgUrl)}','${escHtml(p.judul||'Jadwal Petugas')}')" title="Klik untuk perbesar">
              <img src="${escHtml(imgUrl)}" alt="" style="width:100%;height:100%;object-fit:cover"
                   onerror="this.parentElement.innerHTML='<span style=\\'font-size:9px;color:#888\\'>No Img</span>'">
            </div>
          </td>
          <td>${isAktif ? '<span class="badge badge-green">Aktif</span>' : '<span class="badge badge-gray">Nonaktif</span>'}</td>
          <td><div class="actions">
            <button class="btn btn-icon btn-sm" onclick="openPetugasLightbox('${escHtml(imgUrl)}','${escHtml(p.judul||'Jadwal Petugas')}')" title="Lihat Foto / Zoom" style="color:var(--accent)">
            ...
            ${can('edit', 'jadwal_petugas_gambar') ? `<button class="btn btn-icon btn-sm" onclick="openEditPetugasModal(${p._id})" title="Edit">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
            </button>` : ''}
            <a href="${escHtml(imgUrl)}" target="_blank" download class="btn btn-icon btn-sm" title="Unduh File" style="color:var(--text-secondary)">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
            </a>
            ${can('delete', 'jadwal_petugas_gambar') ? `<button class="btn btn-icon btn-sm" onclick="deletePetugas(${p._id})" title="Hapus" style="color:var(--danger);border-color:rgba(224,82,82,0.3)">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 01-2 2H7a2 2 0 01-2-2V6m3 0V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>
            </button>` : ''}
          </div></td>
        </tr>`;
    ...
    initPagination('petugasTable', 20);
    if (window._applyLabels) window._applyLabels('petugasTable');
}

function applyPetugasFilter() {
    const q = document.getElementById('searchPetugas').value.toLowerCase();
    renderPetugasTable(q ? allPetugas.filter(p => (p.judul||'').toLowerCase().includes(q)) : allPetugas);
}
...
function openAddPetugasModal() {
    ...
    document.getElementById('btnPtSave').innerHTML = 'Upload Jadwal';
    document.getElementById('ptFieldId').value            = '';
    ...
    document.getElementById('ptFileRequired').style.display = 'inline';
    document.getElementById('ptImgPreviewWrap').style.display = 'none';
    ptResetDropZone(true);
...
function openEditPetugasModal(id) {
    ...
    if (p.nama_file) {
        ...
        previewImg.src = cdnBase + '/jadwal_petugas/' + encodeURIComponent(p.nama_file);
...
async function submitPetugasForm() {
    ...
    if (isEdit && !can('edit', 'jadwal_petugas_gambar'))    { toast('Akses Ditolak','Tidak ada izin edit','error'); return; }
    if (!isEdit && !can('create', 'jadwal_petugas_gambar')) { toast('Akses Ditolak','Tidak ada izin upload','error'); return; }
...
// ── Lightbox untuk preview gambar jadwal petugas ────────────────
function openPetugasLightbox(url, title) {
    // Hapus instance lama jika ada
    const existing = document.getElementById('petugasLightbox');
    if (existing) existing.remove();

    const overlay = document.createElement('div');
    overlay.id = 'petugasLightbox';
    overlay.className = 'petugas-lightbox';
    overlay.innerHTML = `
        <div class="plb-backdrop" onclick="closePetugasLightbox()"></div>
        <div class="plb-wrap">
            <button class="plb-close" onclick="closePetugasLightbox()" aria-label="Tutup">×</button>
            <div class="plb-title">${escHtml(title)}</div>
            <img src="${escHtml(url)}" alt="${escHtml(title)}" class="plb-img"
                 onerror="this.style.background='#222';this.style.minHeight='300px'">
            <div class="plb-actions">
                <a href="${escHtml(url)}" target="_blank" rel="noopener" class="plb-btn plb-btn--primary">
                ...
                <a href="${escHtml(url)}" download class="plb-btn">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                    Unduh
                </a>
            </div>
    ...
    requestAnimationFrame(() => overlay.classList.add('petugas-lightbox--open'));
    document.addEventListener('keydown', _plbEscHandler);
}
function _plbEscHandler(e) {
    if (e.key === 'Escape') closePetugasLightbox();
}
function closePetugasLightbox() {
    const lb = document.getElementById('petugasLightbox');
    if (!lb) return;
    lb.classList.remove('petugas-lightbox--open');
    document.body.style.overflow = '';
    document.removeEventListener('keydown', _plbEscHandler);
    setTimeout(() => lb.remove(), 180);
}

// ── Hapus Jadwal Petugas ──────────────────────────────────────────
async function deletePetugas(id) {
    if (!can('delete', 'jadwal_petugas_gambar')) { toast('Akses Ditolak','Tidak ada izin hapus','error'); return; }
    const p = allPetugas.find(x => x._id === id);
    confirmDialog(
        'Hapus Jadwal Petugas?',
