...
function renderTable(data) {
  ...
  tbody.innerHTML = data.map(row => {
    ...
    const ketuaHtml = ketuaFoto
      ? `<div class="foto-cell-sm"><img class="foto-thumb-sm" src="${escHtml(window.PERSON_CDN_URL+'/'+ketuaFoto)}" alt="" onerror="this.style.display='none'"><span>${escHtml(row['Ketua']||'')}</span></div>`
      : `<span>${escHtml(row['Ketua']||'')}</span>`;

    const koorHtml = koorNama
      ? (koorFoto
          ? `<div class="foto-cell-sm"><img class="foto-thumb-sm" src="${escHtml(window.PERSON_CDN_URL+'/'+koorFoto)}" alt="" onerror="this.style.display='none'"><span>${escHtml(koorNama)}</span></div>`
...
function setKoorFotoPreview(foto) {
  ...
  if (foto) { prev.src = window.PERSON_CDN_URL + '/' + foto; prev.style.display = 'block'; plac.style.display = 'none'; }
...
function openEditModal(rowNum) {
  ...
  if (ketuaFoto) { kprev.src=window.PERSON_CDN_URL+'/'+ketuaFoto; kprev.style.display='block'; kplac.style.display='none'; }
