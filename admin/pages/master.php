...
function renderKoordinator() {
  ...
  document.getElementById('bodyKoor').innerHTML = filtered.map(r => {
    ...
    const fotoSrc  = fotoFile ? window.PERSON_CDN_URL + '/' + fotoFile : '';
...
function openKoorModal(id) {
  ...
  if (id) {
    ...
    if (r) {
      ...
      if (r['Foto']) {
        prev.src = window.PERSON_CDN_URL + '/' + r['Foto']; prev.style.display = 'block'; plac.style.display = 'none';
      } else {
        prev.style.display = 'none'; plac.style.display = 'flex';
      }
    }
