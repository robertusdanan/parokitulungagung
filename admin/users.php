...
$abbrMap = [
    'agenda'                => ['Ag', 'Info / Agenda'],
    'dokumen_paroki'        => ['Dk', 'Dokumen Download'],
    'jadwal_petugas_gambar' => ['Jp', 'Jadwal Petugas'],
    'galeri'                => ['Gl', 'Galeri Foto'],
    'romo_paroki'           => ['Rm', 'Romo Paroki'],
    'wilayah'               => ['Wl', 'Profil Wilayah'],
    'asisten_imam'          => ['Ai', 'Asisten Imam'],
    'dpp_bgkp'              => ['Dp', 'DPP & BGKP'],
    'umkm'                  => ['Um', 'UMKM Umat'],
    'media'                 => ['Md', 'Media Manager'],
    'kategorial'            => ['Kt', 'Profil Kategorial'],
    'berita'                => ['Bt', 'Liputan Berita'],
    'kronik'                => ['Krn', 'Kronik SMDTBA'],
    'historia'              => ['Hs', 'Historia Gereja'],
    'master'                => ['Ms', 'Master Data'],
];
...
.perm-table-compact th:last-child {
  border-right: none;
}
.perm-table-compact td {
  padding: 6px 3px;
  border-right: 1px solid var(--border);
  font-weight: 700;
  cursor: help;
}
.perm-table-compact td:last-child {
  border-right: none;
}
.perm-table-compact td.has-access {
  color: var(--accent);
  background: rgba(201, 168, 76, 0.06);
}
.perm-table-compact td.no-access {
    ...
    <table class="data-table" id="dataTable">
      <thead>
        <tr>
          ...
          <th class="hide-mobile">Permissions</th><th>Status</th>
        ...
        <tr>
          ...
          <td class="hide-mobile">
            <?php if (($u['role']??'') === ROLE_SUPERADMIN): ?>
              <span class="badge badge-gold" style="font-size:10px;padding:4px 10px">✓ Semua Akses</span>
            <?php else: ?>
              <table class="perm-table-compact">
                <thead>
                  <tr>
                    <?php foreach ($abbrMap as $key => $info): ?>
                      <th title="<?= e($info[1]) ?>"><?= e($info[0]) ?></th>
                    <?php endforeach; ?>
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <?php foreach ($abbrMap as $key => $info):
                      $rawPerms = getPermissionsMap($u);
                      $hasAccess = array_key_exists($key, $rawPerms);
                      $tooltipText = $info[1];
                      if ($hasAccess) {
                          $userActs = $rawPerms[$key];
                          $actLabels = array_map(fn($a) => PAGE_ACTION_LABELS[$a] ?? $a, $userActs);
                          $tooltipText .= ': ' . implode(', ', $actLabels);
                      } else {
                          $tooltipText .= ': Tidak Ada Akses';
                      }
                    ?>
                      <td class="<?= $hasAccess ? 'has-access' : 'no-access' ?>" title="<?= e($tooltipText) ?>">
                        <?= $hasAccess ? '✓' : '-' ?>
                      </td>
                    <?php endforeach; ?>
                  </tr>
