...
if ($action === 'update') {
    ...
    $newBio       = trim($body['bio']       ?? '');
    $newJabatan   = trim($body['jabatan']   ?? '');
    $newInstagram = trim($body['instagram'] ?? '');
    $newFacebook  = trim($body['facebook']  ?? '');
    $newTwitter   = trim($body['twitter']   ?? '');
    $newWebsite   = trim($body['website']   ?? '');
    ...
    if ($newBio !== '' && mb_strlen($newBio) > 1000) apiJson(['error' => 'Bio maksimal 1000 karakter.'], 400);
    if ($newJabatan !== '' && mb_strlen($newJabatan) > 100) apiJson(['error' => 'Jabatan maksimal 100 karakter.'], 400);
    if ($newInstagram !== '' && mb_strlen($newInstagram) > 150) apiJson(['error' => 'Instagram maksimal 150 karakter.'], 400);
    if ($newFacebook !== '' && mb_strlen($newFacebook) > 150) apiJson(['error' => 'Facebook maksimal 150 karakter.'], 400);
    if ($newTwitter !== '' && mb_strlen($newTwitter) > 150) apiJson(['error' => 'Twitter maksimal 150 karakter.'], 400);
    if ($newWebsite !== '' && mb_strlen($newWebsite) > 200) apiJson(['error' => 'Website maksimal 200 karakter.'], 400);
    ...
    if (array_key_exists('bio', $body))       $updateData['bio']       = $newBio;
    if (array_key_exists('jabatan', $body))   $updateData['jabatan']   = $newJabatan;
    if (array_key_exists('instagram', $body)) $updateData['instagram'] = $newInstagram;
    if (array_key_exists('facebook', $body))  $updateData['facebook']  = $newFacebook;
    if (array_key_exists('twitter', $body))   $updateData['twitter']   = $newTwitter;
    if (array_key_exists('website', $body))   $updateData['website']   = $newWebsite;
    ...
    if (array_key_exists('bio', $body))       $_SESSION['admin_user']['bio']       = $newBio;
    if (array_key_exists('jabatan', $body))   $_SESSION['admin_user']['jabatan']   = $newJabatan;
    if (array_key_exists('instagram', $body)) $_SESSION['admin_user']['instagram'] = $newInstagram;
    if (array_key_exists('facebook', $body))  $_SESSION['admin_user']['facebook']  = $newFacebook;
    if (array_key_exists('twitter', $body))   $_SESSION['admin_user']['twitter']   = $newTwitter;
    if (array_key_exists('website', $body))   $_SESSION['admin_user']['website']   = $newWebsite;

    // Invalidate cache author profile & contributor list
    if (function_exists('cache_del')) {
        cache_del('author_user_full_' . md5($newNama));
        cache_del('author_user_full_' . md5($newUsername));
        if (!empty($self['nama'])) cache_del('author_user_full_' . md5($self['nama']));
        if (!empty($self['username'])) cache_del('author_user_full_' . md5($self['username']));
        cache_del('all_contributors_v1');
    }

    $logger->log($currentUser, 'UPDATE', 'profil',
        'Update profil: ' . ($usernameChanged ? "username → {$newUsername}" : 'data profil & media sosial'));
