    ...
    switch ($section) {
        ...
        case 'umkm':
            ...
            foreach ($rows as $row) {
                $foto = $row['gambar'] ?? '';
                if (!$foto) continue;
                $imgUrl = function_exists('umkmUrl') ? umkmUrl($foto) : normalizeImgUrl($foto, '/umkm');
        ...
        case 'dpp_bgkp':
            ...
            $result = [];
            $seen   = [];
            $personR2prefix = 'person/';          // disimpan di DB sebagai nama file saja
            foreach ($rows as $row) {
                $foto = $row['Foto'] ?? '';
                if (!$foto) continue;
                // Bangun URL publik R2 langsung dari nama file
                $cdnBase = defined('R2_CDN_URL') ? rtrim(R2_CDN_URL, '/') : '';
                $imgUrl  = $cdnBase . '/' . $personR2prefix . ltrim($foto, '/');
        ...
        case 'wilayah':
            ...
            foreach ($rows as $row) {
                ...
                if (!empty($row['Foto'])) {
                    $imgUrl = $cdnBase . '/' . $personR2prefix . ltrim($row['Foto'], '/');
                ...
                if (!empty($row['KoordinatorFoto'])) {
                    $imgUrl = $cdnBase . '/' . $personR2prefix . ltrim($row['KoordinatorFoto'], '/');
                    if (!isset($seen[$imgUrl])) {
                        $seen[$imgUrl] = true;
...
{
    // Foto person sekarang disimpan di Cloudflare R2 (prefix R2_PERSON_PREFIX),
    // dilayani lewat CDN R2_CDN_URL (custom domain img.parokitulungagung.org).
    $personR2 = defined('R2_CDN_URL')
        ? rtrim(R2_CDN_URL, '/') . '/' . trim(defined('R2_PERSON_PREFIX') ? R2_PERSON_PREFIX : 'person/', '/')
        : '';
    $umkmR2 = defined('R2_CDN_URL')
        ? rtrim(R2_CDN_URL, '/') . '/' . trim(defined('R2_UMKM_PREFIX') ? R2_UMKM_PREFIX : 'umkm/', '/')
        : 'umkm/';
    return match ($section) {
        'galeri'       => '/public/galeri/',
        'umkm'         => $umkmR2,
        'dpp_bgkp'     => $personR2,
        'asisten_imam' => $personR2,
        'wilayah'      => $personR2,
        default        => '/',
    };
}
