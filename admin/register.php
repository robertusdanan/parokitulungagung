...
// ── Diteruskan dari tombol "Berkontribusi sebagai Penulis" di artikel,
// supaya setelah daftar → login, user kembali ke halaman artikel semula.
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // ── Rate limiting: maks 5 percobaan registrasi per IP per 10 menit ─
    $ipKey     = 'rf_' . md5($_SERVER['REMOTE_ADDR'] ?? 'x');
    ...
    if (count($attempts) >= 5) {
        ...
        $error   = 'Terlalu banyak percobaan registrasi. Coba lagi dalam ' . ceil($waitSec / 60) . ' menit.';
    } else {
        $attempts[] = $now;
...
<div class="register-page">
  ...
  <div class="register-box">
    ...
    <div class="login-link">
      Sudah punya akun? <a href="/admin<?= $redirectTo ? '?redirect=' . urlencode($redirectTo) : '' ?>">Masuk di sini</a>
    </div>
  </div>
</div>
<script>
