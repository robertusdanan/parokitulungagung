 ...
 * Komponen reusable: picker foto dari R2 CDN (prefix "person/") + upload foto baru
...
<div class="fp-overlay" id="fotoPickerModal">
  ...
  function _renderGrid(list) {
    ...
    grid.innerHTML = list.map(name => {
      const src   = window.PERSON_CDN_URL + '/' + name + '.webp';
      const label = name.replace(/-/g, ' ');
      return `<div class="fp-item" data-name="${name}" onclick="FotoPicker.selectFromGallery('${name}')">
        <img src="${src}" alt="${label}" loading="lazy" onerror="this.src=window.ICON_ROMO_URL||'https://img.parokitulungagung.org/icon/icon_romo.png'">
  ...
  function _updateFooter() {
    ...
    if (_currentSelection) {
      preview.innerHTML = `<div class="fp-selected-preview">
        <img src="${_currentSelection.previewUrl}" alt="" onerror="this.src=window.ICON_ROMO_URL||'https://img.parokitulungagung.org/icon/icon_romo.png'">
      ...
      if (inputEl && inputEl.value) {
        ...
        if (isPersonPhoto) {
          _currentSelection = {
            ...
            previewUrl: window.PERSON_CDN_URL + '/' + name + '.webp',
            source: 'gallery'
          };
        } else if (existing) {
          _currentSelection = {
            ...
            previewUrl: window.PERSON_CDN_URL + '/' + existing,
            source: 'existing'
          };
        }
      }
