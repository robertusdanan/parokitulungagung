...
// ─── GET: ambil riwayat percakapan (untuk restore di halaman lain) ──
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $action = trim($_GET['action'] ?? '');
    if ($action === 'history') {
        $sessionId = trim($_GET['session_id'] ?? '');
        if (empty($sessionId)) {
            jsonOut(400, ['error' => 'session_id wajib diisi']);
        }
...
if ($isGeneralQuestion) {
    ...
    $articleText = buildArticleContext($message, $pageUrl);
...
/**
 * Bangun konteks artikel untuk dikirim ke AI:
 *  1. Hasil pencarian lintas kategori (Berita/Kronik/Historia) berdasarkan
 *     kata kunci di pesan user — TIDAK tergantung halaman yang sedang dibuka.
 *  2. Konten halaman yang sedang dibuka user (jika ada), sebagai tambahan.
 * Hasil pencarian ditaruh di depan supaya tidak terpotong oleh batas
 * panjang konteks di gemini.php/groq.php.
 */
function buildArticleContext(string $message, string $pageUrl): string {
    $parts = [];

    $matches = ArticleReader::search($message);
    if (!empty($matches)) {
        $lines = ['Artikel yang relevan dengan pertanyaan (dari Berita, Kronik, & Historia):'];
        foreach ($matches as $m) {
            $lines[] = "- [{$m['label']}] {$m['judul']} — {$m['ringkas']} (Link: {$m['url']})";
        }
        $parts[] = implode("\n", $lines);
    }
    ...
    if ($pageUrl) {
        ...
        if ($pageText) {
            $parts[] = "Konten halaman yang sedang dibuka user:\n" . $pageText;
        }
    }
