...
(function () {
  ...
  const AVATAR_URL    = 'https://img.parokitulungagung.org/assets/avatar.webp';
  ...
  /* ── STATE MODUL (dipakai lintas fungsi) ───────────────── */
  let greeted = false;

  /* ── RESTORE RIWAYAT PERCAKAPAN ─────────────────────────── */
  // Dipanggil saat halaman dimuat, supaya chat tetap berlanjut
  // di halaman manapun pada device/browser yang sama — bukan
  // mengulang dari 0. Server menyimpan riwayat berdasarkan
  // session_id (localStorage) selama sesi belum timeout (±30 menit).
  async function restoreHistory() {
    try {
      const sid = getSessionId();
      const res = await fetch(`${API_ENDPOINT}?action=history&session_id=${encodeURIComponent(sid)}`);
      if (!res.ok) return;
      ...
      history.forEach(turn => {
        if (turn.role === 'user') {
          appendMsg(buildBubble(turn.content, null, true, null));
        } else {
          appendMsg(buildBubble(turn.content, null, false, turn.source || 'kb'));
        }
      });

      setQuickReplies(INITIAL_QR);
      greeted = true; // jangan tampilkan sapaan awal lagi, ini lanjutan chat
    } catch (err) {
      console.warn('[Chatbot] Gagal memuat riwayat percakapan', err);
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Muat riwayat percakapan (jika ada) sesegera mungkin saat halaman siap.
    // Disimpan sebagai promise supaya openPanel() bisa menunggu selesai dulu
    // sebelum memutuskan perlu menampilkan sapaan awal atau tidak.
    const historyReady = restoreHistory();
    ...
    async function openPanel() {
      ...
      await historyReady; // pastikan status riwayat sudah diketahui dulu
