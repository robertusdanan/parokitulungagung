...
$authorMeta  = function_exists('fetchAuthorUser') ? fetchAuthorUser($art['penulis'] ?? '') : [];
$authorInitial = !empty($art['penulis']) ? strtoupper(substr($art['penulis'], 0, 1)) : '✝';
$metaDesc    = seo_meta_description($art['ringkasan'] ?? $art['konten'] ?? '');

$_root = rtrim($_SERVER['DOCUMENT_ROOT'], '/');

$metaImage = 'https://img.parokitulungagung.org/ogpreview/default.jpg';

if (!empty($art['thumbnail'])) {
    $_thumbRaw  = $art['thumbnail'];
    $_thumbPath = preg_replace('#^https?://[^/]+#', '', $_thumbRaw);
    $_thumbBase = pathinfo(basename($_thumbPath), PATHINFO_FILENAME);
    $metaImage  = ogPreviewUrl($_thumbBase . '.jpg');
}
...
/* ── Profil Penulis Button & Card ────────────────────────── */
.art-author-profile-btn {
  display: inline-flex;
  align-items: center;
  ...
  font-weight: 600;
  color: #7b5914;
  background: linear-gradient(180deg, #fffefb 0%, #faf3e5 100%);
  border: 1px solid rgba(184, 134, 11, 0.4);
  border-radius: 20px;
  text-decoration: none;
  box-shadow: 0 2px 5px rgba(184, 134, 11, 0.08);
  transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1);
  white-space: nowrap;
}
.art-author-profile-btn:hover {
  color: #fff;
  background: linear-gradient(135deg, #b8860b 0%, #946c07 100%);
  border-color: #946c07;
  box-shadow: 0 4px 12px rgba(184, 134, 11, 0.28);
  transform: translateY(-1px);
}
.art-author-profile-btn svg {
  transition: transform 0.2s ease;
}
.art-author-profile-btn:hover svg {
...
.art-author-meta-btn:hover {
  background: #b8860b;
  color: #fff;
  border-color: #b8860b;
}
.art-author-card-footer {
  margin-top: 24px;
  padding: 16px 20px;
  background: #fdfcf9;
...
.art-author-card-footer-inner {
  display: flex;
  align-items: center;
  gap: 16px;
  flex-wrap: wrap;
}
.art-author-card-footer-avatar-wrap {
  flex-shrink: 0;
}
.art-author-card-footer-avatar {
  width: 52px;
  height: 52px;
  border-radius: 50%;
  object-fit: cover;
...
.art-author-card-footer-init {
  width: 52px;
  height: 52px;
  border-radius: 50%;
  background: linear-gradient(135deg, #b8860b 0%, #e6b800 100%);
  color: #fff;
  font-size: 20px;
  font-weight: 700;
  display: flex;
...
.art-author-card-footer-content {
  flex: 1;
  min-width: 180px;
}
.art-author-card-footer-label {
  font-size: 11px;
  text-transform: uppercase;
  letter-spacing: .06em;
  color: #9a836a;
...
.art-author-card-footer-sub {
  font-size: 12px;
  color: #8a7052;
  margin-top: 2px;
}
.art-author-card-footer-bio {
  font-size: 12.5px;
  color: #5a4b39;
  margin-top: 4px;
  line-height: 1.4;
        ...
        <div class="art-sidebar-left-inner">
          <div class="art-author-card" itemscope itemtype="https://schema.org/Person">
            ...
            <?php if (!empty($art['penulis'])): ?>
              ...
              <a href="/penulis/<?= e($penulisSlug) ?>" class="art-author-profile-btn" aria-label="Lihat Profil Penulis <?= e($art['penulis']) ?>">
          ...
          <div class="art-sidebar-ad" style="margin-top:1.5rem;">
            ...
            <ins class="adsbygoogle"
                 ...
                 data-ad-layout-key="-hz+2-8-i+1t"
                 data-ad-client="ca-pub-3946321979629144"
                 data-ad-slot="4488715359"></ins>
            <script>
        ...
        <article class="art-detail-article" itemscope itemtype="https://schema.org/NewsArticle">
          ...
          <header class="art-detail-header">
            ...
            <div class="art-detail-meta">
              <?php if (!empty($art['penulis'])):
                // Slug penulis untuk link ke halaman profil /penulis/{slug}
                // (sama seperti yang dipakai kartu penulis di sidebar desktop)
                $penulisSlugMobile = $penulisSlug ?? trim(preg_replace('/[^a-z0-9]+/', '-', strtolower(strip_tags($art['penulis']))), '-');
              ?>
              <a href="/penulis/<?= e($penulisSlugMobile) ?>"
                 class="art-detail-meta-item art-detail-meta-item--link"
                 itemprop="author" itemscope itemtype="https://schema.org/Person"
                 aria-label="Lihat profil penulis <?= e($art['penulis']) ?>">
                <?php if ($authorPhoto): ?>
                ...
                <div>
                  ...
                  <div class="art-detail-meta-role">Penulis <span class="art-detail-meta-arrow" aria-hidden="true">›</span></div>
                </div>
              </a>
              <a href="/penulis/<?= e($penulisSlugMobile) ?>" class="art-author-meta-btn" aria-label="Profil Penulis <?= e($art['penulis']) ?>">
                Profil Penulis
              </a>
              <?php endif; ?>
              <?php if ($tanggal): ?>
          ...
          $thumbSchemaEntry = [
              '@type'      => 'ImageObject',
              'contentUrl' => artikelImageUrl($art['thumbnail']),
              'name'       => $thumbTitle,
          ...
          <figure class="art-detail-thumb-wrap"<?= $thumbSchemaDesc ? ' itemscope itemtype="https://schema.org/ImageObject"' : '' ?>>
            ...
            <?php endif; ?>
            <img src="<?= e(artikelImageUrl($art['thumbnail'])) ?>"
                 alt="<?= e($thumbAlt) ?>"
                 title="<?= e($thumbTitle) ?>"
          ...
          $konten = $art['konten'] ?? '';
          $konten = preg_replace('#(https?://(www\.)?parokitulungagung\.org)?/img/artikel/#i', 'https://img.parokitulungagung.org/artikel/', $konten);
          ...
          // ── Sisipkan iklan In-Article AdSense di tengah konten artikel ──────
          if (!function_exists('sisipkan_iklan_in_article')) {
              function sisipkan_iklan_in_article(string $html, string $adSlotHtml, int $setelahParagraf = 3): string {
                  // Pecah HTML per paragraf utuh (berdasarkan penutup tag </p>)
                  ...
                  if ($buffer !== '') $paragraphs[] = $buffer; // sisa teks tanpa </p> di akhir

                  $totalP = count($paragraphs);
                  if ($totalP < 2) return $html; // artikel terlalu pendek, jangan sisipkan iklan

                  // Sisip setelah paragraf ke-N, atau di tengah kalau artikelnya pendek
                  $posisi = min($setelahParagraf, (int) floor($totalP / 2));
                  $posisi = max($posisi, 1);

                  $hasil = '';
                  foreach ($paragraphs as $i => $p) {
                      $hasil .= $p;
                      if (($i + 1) === $posisi) $hasil .= $adSlotHtml;
                  }
                  return $hasil;
              }
          }
          ...
          <footer class="art-detail-footer">
            ...
            <?php if (!empty($art['penulis'])): ?>
            <div class="art-author-card-footer">
              <div class="art-author-card-footer-inner">
                <div class="art-author-card-footer-avatar-wrap">
                  <?php if ($authorPhoto): ?>
                  <img src="<?= e($authorPhoto) ?>" alt="<?= e($art['penulis']) ?>" class="art-author-card-footer-avatar" width="52" height="52" loading="lazy" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
                  <div class="art-author-card-footer-init" style="display:none"><?= e($authorInitial) ?></div>
                  <?php else: ?>
                  <div class="art-author-card-footer-init"><?= e($authorInitial) ?></div>
                  <?php endif; ?>
                </div>
                <div class="art-author-card-footer-content">
                  <div class="art-author-card-footer-label">Tentang Penulis</div>
                  <div class="art-author-card-footer-name"><?= e($art['penulis']) ?></div>
                  <?php if (!empty($authorMeta['jabatan'])): ?>
                  <div class="art-author-card-footer-sub"><?= e($authorMeta['jabatan']) ?></div>
                  <?php endif; ?>
                  <?php if (!empty($authorMeta['bio'])): ?>
                  <div class="art-author-card-footer-bio"><?= e(mb_strimwidth($authorMeta['bio'], 0, 160, '...')) ?></div>
                  <?php endif; ?>
                </div>
                <a href="/penulis/<?= e($penulisSlugMobile) ?>" class="art-author-profile-btn" aria-label="Lihat Profil Penulis <?= e($art['penulis']) ?>">
                  <span>Profil Penulis</span>
                  <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                </a>
              </div>
          ...
          <section class="art-related" aria-label="Artikel terkait">
            ...
            <div class="art-related-grid">
              ...
              <a class="art-related-card" href="/artikel/<?= $menu ?>/<?= rawurlencode($relSlug) ?>">
                ...
                <img src="<?= e(artikelImageUrl($rel['thumbnail'])) ?>" alt="<?= e($relJudul) ?>"
                     class="art-related-thumb" loading="lazy" decoding="async" width="300" height="169"
        ...
        <div class="art-sidebar-right-inner">
          ...
          <div class="art-sidebar-related-list">
            ...
            <a class="art-sidebar-related-card" href="/artikel/<?= $menu ?>/<?= rawurlencode($relSlug) ?>">
              <?php if (!empty($rel['thumbnail'])): ?>
              <img src="<?= e(artikelImageUrl($rel['thumbnail'])) ?>" alt="<?= e($relJudul) ?>"
                   class="art-sidebar-related-thumb" loading="lazy" decoding="async" width="54" height="54"
                   onerror="this.src='<?= e(assetUrl('og-preview.webp')) ?>';this.onerror=null;">
              <?php else: ?>
...
if (!empty($rawKonten) && class_exists('DOMDocument')) {
    libxml_use_internal_errors(true);
