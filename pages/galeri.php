...
// ── Sampul album: URL TAMPILAN (langsung dari R2, tanpa watermark) vs
// URL MENTAH (kunci SEO) ──
// URL mentah ('/public/galeri/<file>') dipertahankan HANYA untuk mencocokkan
// data alt-text/caption yang sudah tersimpan di Supabase (image_seo), karena
// kolom itu masih berisi path lama. URL tampilan sekarang mengambil LANGSUNG
// dari Cloudflare CDN (custom domain R2) di bawah prefix _thumbnails/galeri/
// — tidak lagi lewat proxy galeri-cover.php (jadi tanpa watermark, tapi
// lebih cepat karena browser ambil bytes langsung dari R2/CDN).
if (!function_exists('galeriCoverUrls')) {
    function galeriCoverUrls(string $gambar, string $base = ''): array {
        ...
        // Normalisasi: kolom 'Gambar' di database menyimpan 2 format —
        // data lama cuma nama file polos, data baru sudah berupa key R2
        // penuh "_thumbnails/galeri/xxx.webp". Ambil basename-nya saja lalu
        // pasang prefix SEKALI supaya tidak dobel untuk data baru, sekaligus
        // aman dari path traversal (persis pola yang dipakai galeri-cover.php).
        $filename = basename(str_replace('\\', '/', $gambar));
        $display  = r2CdnUrl(R2_GALERI_THUMB_PREFIX . $filename);
...
$seo = [
    ...
    'image'       => !empty($latestItem['Gambar'])
        ? galeriCoverUrls($latestItem['Gambar'], 'https://www.parokitulungagung.org')['display']
        : assetUrl('og-preview.webp'),
...
<head>
  ...
  foreach ($_galSample as $_img) {
      $_src = !empty($_img['Gambar']) ? galeriCoverUrls($_img['Gambar'], $_base)['display'] : '';
      if (!$_src) continue;
      $_obj = [
          '@type'       => 'ImageObject',
          'contentUrl'  => $_src,
  ...
  foreach ($_galSample as $_idx => $_item) {
      $_thumbSrc = !empty($_item['Gambar']) ? galeriCoverUrls($_item['Gambar'], $_base)['display'] : '';
      $_listItems[] = [
          '@type'    => 'ListItem',
          'position' => $_idx + 1,
  ...
  $_galSchema = [
      ...
      'thumbnailUrl' => !empty($_galSample[0]['Gambar'])
          ? galeriCoverUrls($_galSample[0]['Gambar'], $_base)['display']
          : assetUrl('og-preview.webp'),
  ];
...
<div id="galeri-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="galeri-modal-title">
  <div id="galeri-modal">
    ...
    <div class="galeri-modal-body">
      ...
      <a href="#" id="galeri-modal-link" class="galeri-modal-btn-album">
...
<main id="main-content" style="padding:6px">
  ...
  <div class="page-hero">
    <div class="page-hero-icon">
      <img src="<?= iconUrl('icon_square_foto.png') ?>" alt="" loading="lazy" width="40" height="40">
    </div>
      ...
      <div class="galeri-month-body">
        <div class="galeri-grid">
          <?php foreach ($items as $idx2 => $item):
            $gambar   = $item['Gambar'] ?? '';
            $_cover   = galeriCoverUrls($gambar);
            $imgSrcRaw = $_cover['raw'];      // path asli — HANYA untuk kunci lookup SEO
            $imgSrc    = $_cover['display'];  // URL langsung dari R2 CDN — dipakai untuk tampilan

            // ── SEO data dari image_seo Supabase (dicocokkan pakai path asli) ──
            $imgSeo  = getImgSeo($imgSrcRaw, $galeriSeoMap);
            ...
            $galeriDataJS[$cardId] = [
              ...
              'ket'     => $item['Keterangan'] ?? '',
              'foto'    => $item['Foto'] ?? '',
              'link'    => '/galeri/album/' . ($item['id'] ?? 0) . '/' . slugify($item['Judul'] ?? ''),
              'id'      => $item['id'] ?? 0,
              'seo_alt' => $altFinal,
              'seo_cap' => $imgCap,
              'seo_tit' => $titleFinal,
